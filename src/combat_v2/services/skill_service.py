...
_JSON_EFFECT_TO_ENUM: Dict[str, str] = {
    ...
    "add_damage": SkillEffectType.ADD_DAMAGE.value,
    ...
    "block_auras": SkillEffectType.BLOCK_AURAS.value,
    "ep_gain_down": SkillEffectType.EP_GAIN_DOWN.value,
    "ep_gain_up": SkillEffectType.EP_GAIN_UP.value,
...
class SkillService:
    ...
    def __init__(self, data_loader, resource_service,
        ...
        self._caster_buffs_active_during_damage: set = set()  # caster的buff在damage效果执行时已存在（用于attack_limited清理区分前后付与）
    ...
    def _execute_trigger_actions_inline(self, actions: list, battlefield: BattlefieldState,
        ...
        for action in actions:
            ...
            if getattr(owner, 'is_confused', False) or getattr(owner, 'is_genwaku', False):
                _log.info("[INLINE_EXEC] %s: SKIPPED PS[%s] (confused=%s genwaku=%s)",
            ...
            saved_pre_scanned_non_damage_enemy_targets = list(getattr(self, '_pre_scanned_non_damage_enemy_targets', []))
            ...
            self._pre_scanned_non_damage_enemy_targets = saved_pre_scanned_non_damage_enemy_targets
    ...
    def _evaluate_global_condition(self, gc, caster: UnitState,
        ...
        支持 type=condition_list 时按mode聚合子条件。
        """
        if not gc or not isinstance(gc, dict):
            return True
        gc_type = gc.get('type')

        # condition_list / and / or: 聚合多个子条件
        # 支持 type='condition_list'(mode='all'/'any') / type='and' / type='or'
        # 'and' 与 block_condition 的 'and' 语义一致（如120158 たまには子供っぽく）
        if gc_type in ('condition_list', 'and', 'or'):
            if gc_type == 'condition_list':
                mode = gc.get('mode', 'all')
            elif gc_type == 'and':
                mode = 'all'
            else:  # 'or'
                mode = 'any'
        ...
        if gc_type == 'round_number':
            ...
            if not _eval_block_condition(cur_round, gc_op, gc_val):
                _log.info("[SKILL_GC] %s: [%s] round_number %d %s %d not met",
                          caster.name, skill_name, cur_round, gc_op, gc_val)
                return False
            ...
            pct = gc.get('pct', 0)
            from src.entities_v2.enums import Side as _SideGC
            team = battlefield.friend_team if caster.side == _SideGC.ALLY else battlefield.enemy_team
            found = any(
                u.is_alive and u.max_hp > 0 and (u.current_hp / u.max_hp * 100.0) < pct
                for u in team
            )
            if not found:
                _log.info("[SKILL_GC] %s: [%s] ally_hp_below_exists pct=%.0f: no ally below threshold, blocked",
                          caster.name, skill_name, pct)
                return False
        elif gc_type == 'ally_alive_other_than_self_exists':
            # 存在自身以外的存活友方时条件满足（如120154「自身以外の味方が生存」）
            from src.entities_v2.enums import Side as _SideGC2
            team = battlefield.friend_team if caster.side == _SideGC2.ALLY else battlefield.enemy_team
            found = any(u.is_alive and u.unit_id != caster.unit_id for u in team)
            if not found:
                _log.info("[SKILL_GC] %s: [%s] ally_alive_other_than_self_exists: no other alive ally, blocked",
                          caster.name, skill_name)
                return False
    ...
    def _check_skill_global_condition(self, resolved, caster: UnitState,
                                       battlefield: BattlefieldState) -> bool:
        """检查技能的global_condition是否满足。

        返回True表示条件满足（或无global_condition），技能可发动；
        返回False表示条件不满足，技能应被跳过。
        用于select_skill阶段提前过滤不满足global_condition的AS技能，
        避免选中后因global_condition失败而浪费行动（如120152逆転の方策持有深慮时）。
    ...
    def select_skill(self, unit: UnitState) -> Optional[int]:
        ...
        is_genwaku = getattr(unit, 'is_genwaku', False)
        is_restricted = is_confused or is_genwaku
        ...
        for sid in unit.skills:
            ...
            if resolved.skill_type == 1 and _is_usable(sid):
                if is_restricted and not self._skill_has_damage_effect(sid, unit.skill_levels.get(sid, 1)):
                    _log.info("[SKILL_SEL] %s: RESTRICTED -> skip AS [%s] (id=%d, no damage effect)",
                              unit.name, resolved.name, sid)
                    continue
                if not self._check_skill_global_condition(resolved, unit, self._battlefield):
                    _log.info("[SKILL_SEL] %s: skip AS [%s] (id=%d, global_condition not met), trying next",
                              unit.name, resolved.name, sid)
                    continue
        ...
        _log.info("[SKILL_SEL] %s: no usable skill -> standby%s",
                  unit.name, " (RESTRICTED)" if is_restricted else "")
        return None
    ...
    def execute_skill(self, caster: UnitState, skill_id: int,
        ...
        self._current_main_damage_power = None  # 技能级别：当前主damage effect的power值，供splash_to_adjacent enchant引用「同威力」
        # after_ps_receivedトリガー用: PS技能の非damage効果対象を記録
        # PS技能の場合のみ意味を持つ、AS/EXでは使用されない
        self._ps_received_targets = []
        # 現在の技能がPSかを判定(skill_type=2)
        self._current_skill_is_ps = False
        try:
            _skill_meta = self.data_loader.get_skill_by_id(skill_id) if self.data_loader else None
            if _skill_meta and _skill_meta.skill_type == SkillType.PS.value:
                self._current_skill_is_ps = True
        except Exception:
            pass
        ...
        # execute_condition: 技能执行前置条件检查（如AS1 120121 HP>=60%才能发动）
        # 条件不满足时直接返回fail，不消耗资源不执行任何效果
        try:
            _parsed = self.data_loader.get_parsed_skill_data(skill_id) if self.data_loader else None
            if _parsed:
                _exec_cond = _parsed.get('execute_condition')
                if _exec_cond:
                    if not self._check_execute_condition(caster, _exec_cond, battlefield):
                        _log.info("[SKILL_EXEC] %s: [%s] execute_condition FAILED -> SKIP",
                                  caster.name, resolved.name)
                        result["error"] = "execute_condition not met"
                        return result
        except Exception as _e:
            _log.warning("[SKILL_EXEC] execute_condition check error: %s", _e)
        ...
        self._caster_buffs_active_during_damage = set()
        ...
        if _has_damage_effects:
            ...
            "enemy_column_lowest_mark_count",
                                ...
                                elif _pre_target_type in ("enemy_column", "ally_column"):
                                    # enemy_column/ally_column必须使用COLUMN范围，而非display_target_range
                                    # display_target_range可能与实际索敌类型不符（如210099衣擦れの調べ
                                    # display_target_range=LINE但effect target_type=enemy_column），
                                    # 导致prescan目标与实际damage目标不一致，触发器条件误判
                                    # （如代助一避误触发bug：prescan选了LINE目标含PS持有者正前方单位，
                                    #   实际damage选COLUMN目标不含该单位）
                                    _pre_range = self._resolve_target_range(_pre_target_type)  # COLUMN
                                else:
                                    # 优先使用技能元数据中的display_target_range，而非从effect target_type推导
                                    # 修复「見切り構め・素」bug：effect.target_type=enemy_single但实际是三体技能
                                    _pre_range = _skill_display_range if _skill_display_range is not None else self._resolve_target_range(_pre_target_type)
                                _pre_tso = type('obj', (object,), {
                                    'display_target_type': self._resolve_target_type(_pre_target_type),
                                    'display_target_range': _pre_range,
                            ...
                            for _pt in _pre_targets:
                                if _pt.unit_id not in _seen_ids and _pt.is_alive:
                                    ...
                                    _is_primary_target_type = (
                                        ...
                                        or _pre_target_type in ("enemy_column_furthest", "enemy_column_mark_priority", "enemy_column_highest_atk", "enemy_column_lowest_mark_count")
                                    )
            ...
            # 预扫描非伤害敌方效果目标（如spd_down的enemy_all目标）
            # 伤害+效果技能中，非伤害效果的目标也视为"被攻击"，guard/evasion PS应触发。
            # 这些目标在L2443处合并到all_block_targets，使所有触发器条件
            # (target_is_self/target_is_front_ally等)能检查到全部敌方效果目标。
            # 纯效果技能（无damage）不执行此预扫描（外层if _has_damage_effects守卫）。
            # 例如 シャル・ウィ・ダンス？(210100): damage→enemy_single_lowest_hp_ratio(1体),
            #   spd_down→enemy_all(5体)。ノーブル・グレイス是spd_down目标但非damage目标，
            #   合并后all_block_targets含全部5体，アイギスランパード/代助一避均可触发。
            _seen_nd_ids = set(_pt.unit_id for _pt in self._pre_scanned_cover_candidates)
            ...
            if self._pre_scanned_non_damage_enemy_targets:
                _log.info("[NON_DMG_ENEMY_TARGETS] %s: pre-scanned non-damage enemy targets: %s",
        ...
        for block in resolved.effect_blocks:
            ...
            # 收集on_crit block：支持直接type=on_crit或and条件中包含on_crit子条件
            # (如120077 Block2: and=[on_crit, active_level_min, mark_count])
            _is_on_crit_block = False
            if block_condition and isinstance(block_condition, dict):
                if block_condition.get('type') == 'on_crit':
                    _is_on_crit_block = True
                elif block_condition.get('type') == 'and':
                    sub_conds = block_condition.get('conditions', [])
                    if any(sc.get('type') == 'on_crit' for sc in sub_conds if isinstance(sc, dict)):
                        _is_on_crit_block = True
            if _is_on_crit_block:
        ...
        for block_idx, block in enumerate(resolved.effect_blocks):
            ...
            if block_condition and isinstance(block_condition, dict):
                ...
                if cond_type == 'target_survived':
                    ...
                    if primary_target is not None:
                        ...
                        if is_dead:
                            ...
                            continue
                elif cond_type == 'any_target_survived':
                    # any_target_survived：检查前序block的所有攻撃対象中是否有任意1体存活
                    # 如EX 110042 Lv11+「攻撃後に敵が生存していた場合」
                    all_attacked = getattr(self, '_skill_all_attacked_targets', []) or []
                    if all_attacked:
                        any_alive = False
                        for t in all_attacked:
                            if self._tactical_exercise_mode:
                                if t.is_alive:
                                    any_alive = True
                                    break
                                ...
                                if t.current_hp > 0:
                                    ...
                                    break
                        if not any_alive:
                            _log.info("[SKILL_EXEC] %s: skipping block %d (any_target_survived: no targets alive)",
                                      caster.name, block.block_id)
                            continue
                    else:
                        _log.info("[SKILL_EXEC] %s: skipping block %d (any_target_survived: no previous targets)",
                                  caster.name, block.block_id)
                        continue
                elif cond_type == 'target_hp_below':
                    # target_hp_below：检查前序block主目标的攻撃前HP割合が閾値以下か
                    # 使用_pre_damage_hp确保攻撃前HP（如AS2 120096「対象のHPが35%以下だった場合」）
                    primary_target = getattr(self, '_last_primary_target', None)
                    if primary_target is None:
                        prev_targets = getattr(self, '_prev_block_damage_targets', {})
                        if prev_targets:
                            ...
                            primary_target = prev_targets[first_key][0] if prev_targets[first_key] else None
                    if primary_target is not None:
                        pre_dmg_hp = getattr(self, '_pre_damage_hp', {}).get(primary_target.unit_id, primary_target.current_hp)
                        hp_pct = pre_dmg_hp / primary_target.max_hp * 100 if primary_target.max_hp > 0 else 100
                        threshold = block_condition.get('value', block_condition.get('pct', 0))
                        if hp_pct > threshold:
                            _log.info("[SKILL_EXEC] %s: skipping block %d (target_hp_below: hp=%.1f%% > %.0f%%)",
                    ...
                    if _active_level < _min_level:
                        ...
                        continue
                elif cond_type == 'active_level_max':
                    # 直接active_level_max条件（非and组合）：技能等级 > value 时跳过
                    _active_level = caster.skill_levels.get(skill_id, 1)
                    _max_level = block_condition.get('value', 0)
                    if _active_level > _max_level:
                        _log.info("[SKILL_EXEC] %s: skipping block %d (active_level_max: %d > %d)",
                                  caster.name, block.block_id, _active_level, _max_level)
                        continue
                    ...
                    if not drawn:
                        _log.info("[SKILL_EXEC] %s: skipping block %d (drawn_target_is_enemy: no drawn targets)",
                                  caster.name, block.block_id)
                        continue
                    matched = [(u, t) for u, t in drawn if t == 'enemy']
                    if not matched:
                    ...
                    if not drawn:
                        _log.info("[SKILL_EXEC] %s: skipping block %d (drawn_target_is_ally: no drawn targets)",
                                  caster.name, block.block_id)
                        continue
                    matched = [(u, t) for u, t in drawn if t == 'ally']
                    if not matched:
                    ...
                    # 检查之前伤害块的目标是否有异常状态（炎上/毒/凍結/眩暈/黑暗/混乱/幻惑）
                ...
                elif cond_type == 'self_has_status_ailment':
                    # 检查施法者自身是否有异常状态（炎上/毒/凍結/眩暈/黑暗/混乱/幻惑）
                    # 参考 セントリストア(130061) Block2「自身に状態異常が付与されている場合、自身にかけられたデバフを全て解除する」
                    STATUS_AILMENT_TYPES = {"knockout", "conflagration", "poison", "freeze",
                                            "darkness", "confusion", "genwaku"}
                    has_ailment = any(
                        d.effect_type.lower() in STATUS_AILMENT_TYPES for d in caster.debuffs
                    )
                    if not has_ailment:
                        _log.info("[SKILL_EXEC] %s: skipping block %d (self_has_status_ailment: no status ailment on self)",
                                  caster.name, block.block_id)
                        continue
                    ...
                    op = block_condition.get('operator', '>=')
                    val = block_condition.get('value', 0)
                    target_scope = block_condition.get('target_scope', 'attacked')
                    # per-target独立判断：检查技能已攻击的所有目标单位，逐个检查mark数是否达标
                    # 必须使用_skill_all_attacked_targets（技能级累积），而非_block_damage_targets，
                    # 因为条件检查发生在block重置之前，_block_damage_targets可能残留上一个mark_count
                    # block缓存填充的达标目标子集（如静かな時を Block3时_block_damage_targets仍为
                    # Block2填充的[e1]，导致e2被遗漏）
                    check_targets = []
                    seen_ids = set()
                    if target_scope == 'enemy':
                        # 显式指定检查敌方队伍（如PS2 on_turn_start无伤害但需检查敌方mark）
                        from ...entities_v2.enums import Side as _SideMC
                        opposing_team = (battlefield.friend_team if caster.side == _SideMC.ENEMY
                                         else battlefield.enemy_team)
                        for u in opposing_team:
                            if u.is_alive and u.unit_id not in seen_ids:
                                seen_ids.add(u.unit_id)
                        ...
                        if not check_targets:
                            # _skill_all_attacked_targets为空时（如mark_count在首个block），
                            # 回退到_block_damage_targets或caster
                            bdt = self._block_damage_targets if hasattr(self, '_block_damage_targets') and self._block_damage_targets else {}
                            for units in bdt.values():
                                for u in units:
                                    if u.unit_id not in seen_ids:
                    ...
                    # 收集所有达标目标（per-target独立判断，如静かな時を：每个被攻击目标独立判断凶兆数）
                    matched_targets = []
                    for check_unit in check_targets:
                        ...
                        if _eval_block_condition(mark_count, op, val):
                            ...
                            _log.info("[SKILL_EXEC] %s: mark_count '%s' on %s = %d %s %d -> FAIL (target excluded)",
                                      caster.name, mark_name, check_unit.name, mark_count, op, val)
                    if not matched_targets:
                        target_names = [u.name for u in check_targets]
                        _log.info("[SKILL_EXEC] %s: skipping block %d (mark_count '%s' on %s: no target met %s %d)",
                                  caster.name, block.block_id, mark_name, target_names, op, val)
                        continue
                    # 记录所有达标目标，供后续效果使用（per-target过滤范围型target_type）
                    self._mark_condition_target = matched_targets[0]
                    self._mark_condition_targets = matched_targets
                    self._mark_count_per_target = True
                    ...
                    if not check_targets:
                        ...
                        if trigger_attacker and trigger_attacker.is_alive:
                            ...
                            # 如enemy_all → 检查所有敌方（敌方施法者的"敌"为friend_team）
                            first_effect = next((e for e in block.effects), None)
                            if first_effect and getattr(first_effect, 'target_type', '') == 'enemy_all':
                                from ...entities_v2.enums import Side as _SideTHM
                                opposing_team = (battlefield.friend_team
                                                 if caster.side == _SideTHM.ENEMY
                                                 else battlefield.enemy_team)
                                check_targets = [u for u in opposing_team if u.is_alive]
                            else:
                                check_targets = [caster]
                    condition_met = False
                    matched_targets = []
                ...
                elif cond_type == 'caster_front_row':
                    # 施法者处于前排时执行（如 130163 スイッチ・ポジション Block1: 前排时获得闪避+即死回避）
                    from ...entities_v2.enums import Position as _PosCfr
                    front_positions = {
                        _PosCfr.ALLY_LEFT_FRONT, _PosCfr.ALLY_CENTER_FRONT, _PosCfr.ALLY_RIGHT_FRONT,
                        _PosCfr.ENEMY_LEFT_FRONT, _PosCfr.ENEMY_CENTER_FRONT, _PosCfr.ENEMY_RIGHT_FRONT,
                    }
                    if caster.position not in front_positions:
                        _log.info("[SKILL_EXEC] %s: skipping block %d (caster_front_row: position=%s not front)",
                                  caster.name, block.block_id, caster.position)
                        continue
                    _log.info("[SKILL_EXEC] %s: block %d caster_front_row PASS (position=%s)",
                              caster.name, block.block_id, caster.position)
            ...
            # Block级别aura目标缓存：同一block内相同target_type的aura effect应使用相同目标，
            # 避免前一个effect的buff（如atk_up）改变属性排序导致后续effect目标漂移
            self._block_aura_target_cache = {}
            # 若mark_count/target_has_mark条件命中目标，将目标填入所有效果使用的目标类型
            # 但不覆盖enemy_all/ally_all等范围型target_type（如ポストリュード：damage应打全体而非仅乱調持有者）
            # 例外: mark_count条件需per-target过滤范围型target_type（如静かな時を：enemy_all只作用于达标目标）
            mark_targets = getattr(self, '_mark_condition_targets', None)
            mark_target = getattr(self, '_mark_condition_target', None)
            mark_count_per_target = getattr(self, '_mark_count_per_target', False)
            if mark_targets:
                for effect in block.effects:
                    et = getattr(effect, 'target_type', None)
                    if et and et not in self._block_damage_targets:
                        if et in ('enemy_all', 'ally_all', 'enemies') and not mark_count_per_target:
                            continue  # 范围型不覆盖（mark_count条件除外）
                ...
                self._mark_count_per_target = False
            ...
            for effect in block.effects:
                ...
                if effect.effect_type == "damage":
                    ...
                    elif target_identifier == "trigger_attack_target":
                        # PS1 130129: 指向触发PS的AS/EX技能的第一优先索敌位（primary_target）
                        primary_target = getattr(self, '_primary_target', None)
                        if primary_target and primary_target.is_alive:
                            self._block_damage_targets[effect.target_type] = [primary_target]
                            _log.info("[SKILL_EXEC] %s: using trigger_attack_target=%s as damage target",
                                      caster.name, primary_target.name)
                            continue
                        # primary_target not available, fall through to default targeting
                    ...
                    if effect.target_type not in self._block_damage_targets:
                        if effect.target_type == "attacked_targets":
                        ...
                        elif effect.target_type == "enemy_row_others":
                            # enemy_row_others: 前block的主目标所在横一列中的其他敌方（排除主目标自身）
                            # 参考 スタンサプレッション(120054) Block2「対象と同じ横一列にいる他の敵」
                            # 主目标来源优先级：_last_primary_target > _prev_block_damage_targets中第一个目标
                            primary_target = getattr(self, '_last_primary_target', None)
                            if primary_target is None or not primary_target.is_alive:
                                # 回退：从_prev_block_damage_targets找第一个目标
                                prev_targets = getattr(self, '_prev_block_damage_targets', {})
                                for _pk, _pv in prev_targets.items():
                                    if _pv:
                                        primary_target = _pv[0]
                                        break
                            if primary_target is None or not primary_target.is_alive:
                                self._block_damage_targets[effect.target_type] = []
                                _log.info("[SKILL_EXEC] %s: enemy_row_others: no primary_target available, empty",
                                          caster.name)
                            else:
                                # 获取所有敌方候选，筛选同row且排除主目标
                                from ...entities_v2.enums import Side as _Side
                                enemy_side_enum = _Side.ENEMY if caster.side == _Side.ALLY else _Side.ALLY
                                all_enemies = [u for u in battlefield.get_alive_units(enemy_side_enum)]
                                anchor_is_front = self.target_service._is_front_row(primary_target)
                                row_others = [u for u in all_enemies
                                              if self.target_service._is_front_row(u) == anchor_is_front
                                              and u.unit_id != primary_target.unit_id]
                                self._block_damage_targets[effect.target_type] = row_others
                                _log.info("[SKILL_EXEC] %s: enemy_row_others: primary=%s (%s row) -> %d others: %s",
                                          caster.name, primary_target.name,
                                          "FRONT" if anchor_is_front else "BACK",
                                          len(row_others), [t.name for t in row_others])
                            ...
                            if effect_flags_block.get('lowest_hp_priority') and target_count > 1:
                                # lowest_hp_priority: 按残HP升序排序（如 AS2 120127「残HP少ない順」）
                                # 取前 target_count 个，而非按距离选 primary
                                # tiebreaker: 同HP时按位置距离（_get_sort_key，最近优先）
                                primary_skill_obj_for_dist = type('obj', (object,), {
                                    'display_target_type': self._resolve_target_type(effect.target_type),
                                    'display_target_range': 1,
                                    'display_target_priority': 0,
                                })()
                                primary_for_dist = self.target_service.select_targets(
                                    primary_skill_obj_for_dist, caster, battlefield
                                )
                                primary_for_dist = primary_for_dist[0] if primary_for_dist else caster
                                enemies.sort(key=lambda u: (u.current_hp, self._get_distance_key(primary_for_dist, u)))
                                self._block_damage_targets[effect.target_type] = enemies[:target_count]
                                _log.info("[SKILL_EXEC] %s: custom target select: lowest_hp_priority=True count=%d targets=%s",
                                ...
                                # target_count > 1 但没有 lowest_hp_priority
                                # front_priority: 前列優先（如 130161 PS1「前列優先で敵2体」）
                                # 排序: 前排(0) < 后排(1)，同排内按距施法者最近优先
                                if effect_flags_block.get('front_priority'):
                                    enemies.sort(key=lambda u: (
                                        1 if self.target_service._is_back_row(u) else 0,
                                        self.target_service._get_sort_key(caster, u)
                                    ))
                                    self._block_damage_targets[effect.target_type] = enemies[:target_count]
                                    _log.info("[SKILL_EXEC] %s: front_priority target select: count=%d targets=%s",
                                              caster.name, target_count,
                                              [t.name for t in self._block_damage_targets[effect.target_type]])
                                else:
                                    # 默认：先选主目标，再按距离选最近目标
                    ...
                    # 注意: enemy_column_highest_atk / enemy_row_highest_atk は列/行選択フィルタで別途処理するため除外
                    dmg_targets = self._block_damage_targets.get(effect.target_type, [])
                    if effect.target_type and "highest_atk" in effect.target_type and "column_highest_atk" not in effect.target_type and "row_highest_atk" not in effect.target_type and dmg_targets:
                    ...
                    elif effect.target_type == "enemy_single_lowest_max_hp" and dmg_targets:
                        best = self.target_service.select_min_with_stealth(
                            dmg_targets,
                            key_func=lambda u: (u.max_hp, self.target_service._get_sort_key(caster, u)),
                            consume=True
                        )
                        dmg_targets = [best] if best else []
                        self._block_damage_targets[effect.target_type] = dmg_targets
                    ...
                    elif effect.target_type == "enemy_column_lowest_mark_count" and dmg_targets:
                        # 优先选择指定mark数量最少的敌方所在列（前后列/纵列）
                        # 用于AS2 120122 容疑者を捕捉！「風紀」の数が最も少ない敵を優先
                        # mark数相同时按距离最近 tiebreak
                        mark_name = effect_flags_block.get('mark_priority', '風紀')
                        def _mark_count(u):
                        ...
                        anchor = self.target_service.select_min_with_stealth(
                            dmg_targets,
                            key_func=lambda u: (_mark_count(u), self._get_distance_key(caster, u)),
                            consume=True
                        )
                        if anchor is None:
                            anchor_col = -1
                    ...
                    # 记录enemy_row_highest_atk的主目标（最高ATK敵），供后续block的enemy_row_others引用
                    if effect.target_type == "enemy_row_highest_atk" and effect.target_type in self._block_damage_targets:
                        row_list = self._block_damage_targets[effect.target_type]
                        if row_list:
                            self._last_primary_target = max(row_list, key=lambda u: u.attack)
                            _log.info("[SKILL_EXEC] %s: recorded _last_primary_target (enemy_row_highest_atk)=%s (atk=%d)",
                                      caster.name, self._last_primary_target.name, self._last_primary_target.attack)
        ...
        # Only consume if this skill has damage effects. Non-damage skills (e.g. PS buffs
    ...
    # effect_type → _apply_xxx 方法名分发表（类级别常量）
    # 使用方法名字符串而非方法引用，因为类定义时方法尚未绑定
    # 注意：
    #   - reset_cooldown 签名特殊（无 battlefield 参数），不放入此表，在 _apply_effect 中单独处理
    #   - guard 同时存在于 _get_buff_types() 中，但需优先走 _apply_guard，
    #     因此本表（含 guard）的查找必须早于 buff/debuff 动态类型判断
    _EFFECT_DISPATCH = {
        "damage": "_apply_damage",
        "heal": "_apply_heal",
        "recover": "_apply_heal",
        "add_status": "_apply_add_status",
        "guard": "_apply_guard",
        "consume_hp": "_apply_consume_hp",
        "hp_ratio_damage": "_apply_hp_ratio_damage",
        "heal_link": "_apply_heal_link",
        "lifesteal": "_apply_lifesteal",
        "shield_from_damage": "_apply_shield_from_damage",
        "damage_special": "_apply_damage_special",
        "remove_debuff": "_apply_remove_debuff",
        "remove_all_buffs": "_apply_remove_all_buffs",
        "remove_all_debuffs": "_apply_remove_all_debuffs",
        "remove_buff": "_apply_remove_buff",
        "remove_buff_by_type": "_apply_remove_buff_by_type",
        "block_buff_by_type": "_apply_block_buff_by_type",
        "stealth": "_apply_stealth",
        "split_heal_by_damage": "_apply_split_heal_by_damage",
        "skill_power_down": "_apply_skill_power_down",
        "sub_unit": "_apply_sub_unit",
        "remove_mark": "_apply_remove_mark",
        "remove_shield": "_apply_remove_shield",
        "remove_sub_unit": "_apply_remove_sub_unit",
        "cover": "_apply_cover",
        "reflect_damage": "_apply_reflect_damage",
        "modify_pp": "_apply_modify_pp",
        "damage_link": "_apply_damage_link",
        "remove_damage_links": "_apply_remove_damage_links",
        # 110072 ミッドサマー・ラブ: 随机抽取目标
        "random_draw": "_apply_random_draw",
        # 130160 サマータイム・ロマンス: HP阈值减伤 buff
        "dmg_taken_down_threshold": "_apply_dmg_taken_down_threshold",
    }

    # server_script 类型直接跳过（无对应 handler）
    _SERVER_SCRIPT_TYPES = frozenset({"server_script_instant", "server_script_aura"})
    ...
    def _apply_effect(self, caster: UnitState, effect, battlefield: BattlefieldState) -> Optional[Dict]:
        ...
        # after_ps_received トリガー用: PS技能の非damage効果の対象を記録
        # damage型効果は含めない（「受けた」=buff/heal等の受動的効果と解釈）
        # 記録はPS技能実行終了後にbattle_flow_controllerが発火
        if self._current_skill_is_ps and etype not in (
            "damage", "hp_ratio_damage", "damage_special", "consume_hp",
            "lifesteal", "shield_from_damage",
        ):
            self._record_ps_received_targets_for_effect(caster, effect, battlefield)
        ...
        if getattr(caster, 'is_confused', False) or getattr(caster, 'is_genwaku', False):
            _CONFUSION_WHITELIST = {"damage", "hp_ratio_damage", "damage_special", "consume_hp"}
            if etype not in _CONFUSION_WHITELIST:
                _log.info("[RESTRICTED] %s: effect %s FAIL (confused/genwaku, only damage effects allowed)",
                          caster.name, etype)
                return None

        # server_script 类型直接跳过（无对应 handler）
        if etype in self._SERVER_SCRIPT_TYPES:
            _log.info("[EFFECT] %s: skipped server_script type=%s", caster.name, etype)
            return None

        # damage 效果特殊前置：记录执行前 caster 的 buff_ids
        # （用于 attack_limited 清理时区分前后付与；修复110010 ヴィヴァーチェ的
        #   dmg_dealt_up 在 damage 后付与时被错误消耗的 bug）
        if etype == "damage":
            for _b in caster.buffs:
                self._caster_buffs_active_during_damage.add(_b.buff_id)

        # reset_cooldown 签名特殊（无 battlefield 参数），单独处理
        if etype == "reset_cooldown":
            return self._apply_reset_cooldown(caster, effect)

        # 静态字典分发
        # 注意：guard 同时存在于 _get_buff_types() 中，但需优先走 _apply_guard，
        #       因此 _EFFECT_DISPATCH（含 guard）的查找必须早于 buff/debuff 动态类型判断
        handler_name = self._EFFECT_DISPATCH.get(etype)
        if handler_name:
            return getattr(self, handler_name)(caster, effect, battlefield)

        # 动态类型集合：resource / buff / debuff
        # ダメージリンク系（damage_link / remove_damage_links）已在静态表中处理
        if etype in self._get_resource_types():
            return self._apply_resource(caster, effect, battlefield)
        if etype in self._get_buff_types():
            return self._apply_aura(caster, effect, battlefield, is_debuff=False)
        if etype in self._get_debuff_types():
    ...
    def _record_ps_received_targets_for_effect(self, caster: UnitState, effect,
                                                battlefield: BattlefieldState) -> None:
        """PS技能の非damage効果の対象unitを_ps_received_targetsに記録(130040用)

        target_typeに応じて対象を予測し、casterと同陣営の対象(=味方)のみ記録。
        trigger_attacker/triggered_by等の特殊identifierも考慮。
        記録は発火時の重複排除のため、unit_idで管理。
        """
        try:
            tt = effect.target_type
            tid = getattr(effect, 'target_identifier', None)
            # self対象は確実
            if tt == "self":
                self._add_ps_received_target(caster)
                return
            # trigger_attacker identifier: 現在のAS攻撃者
            if tid == "trigger_attacker":
                ta = getattr(self, '_trigger_attacker', None)
                if ta and ta.is_alive:
                    self._add_ps_received_target(ta)
                return
            # triggered_by identifier: トリガー発火元(被攻撃者等)
            if tid == "triggered_by":
                tb = getattr(self, '_trigger_triggered_by', None)
                if tb and tb.is_alive:
                    self._add_ps_received_target(tb)
                return
            # nearest_ally identifier: 直近味方
            if tid == "nearest_ally":
                # _apply_aura内で実際に選定されるが、ここでは簡易予測(最も近い1体)
                from src.entities_v2.enums import Side as _SideNA2
                team = battlefield.friend_team if caster.side == _SideNA2.ALLY else battlefield.enemy_team
                ...
                if candidates and self.target_service:
                    ...
                    if nearest:
                        self._add_ps_received_target(nearest)
                return
            # target_typeに基づく選定
            if not tt or tt == "unknown":
                return
            from src.entities_v2.enums import DisplayTargetRange
            tso = type('obj', (object,), {
                'display_target_type': self._resolve_target_type(tt),
                'display_target_range': self._resolve_target_range(tt),
                'display_target_priority': self._current_skill_priority,
                'target_type_name': tt,
            })()
            if self.target_service:
                targets = self.target_service.select_targets(tso, caster, battlefield)
                if targets:
                    for t in targets:
                        if t.is_alive:
                            self._add_ps_received_target(t)
        except Exception as _e:
            _log.info("[PS_RECEIVED_RECORD] %s: error recording targets for effect_type=%s: %s",
                      caster.name, getattr(effect, 'effect_type', '?'), _e)

    def _add_ps_received_target(self, unit: UnitState) -> None:
        """PS受け取り対象を追加(重複排除)"""
        if not unit or not unit.is_alive:
            return
        existing_ids = {u.unit_id for u in self._ps_received_targets}
        if unit.unit_id not in existing_ids:
            self._ps_received_targets.append(unit)
    ...
    def _apply_damage(self, caster: UnitState, effect, battlefield: BattlefieldState) -> Optional[Dict]:
        ...
        # P0-3 Phase 3: 目标选择 + 过滤独立为方法
        targets = self._apply_damage_select_and_filter_targets(caster, effect, battlefield)
        if targets is None:
            return None

        # is_using_cached: 目标是否来自 _block_damage_targets 缓存
        # 缓存目标可能是已死亡目标，需跳过 OVERKILL 分支（与原 _apply_damage_select_and_filter_targets 内部判断一致）
        _cached = getattr(self, '_block_damage_targets', None)
        if _cached is not None and isinstance(_cached, dict) and effect.target_type in _cached:
            is_using_cached = True
        elif _cached is not None and isinstance(_cached, list):
            is_using_cached = True
        else:
            is_using_cached = False

        # P0-3 Phase 3: BEFORE_ENEMY_AS_ATTACK/BEFORE_ANY_ATTACKED/BEFORE_AS_ATTACKED 三时机PS触发
        self._apply_damage_fire_before_attack_triggers(caster, targets, battlefield)
        ...
        # effect_flags 在主方法中供后续 hp_scaling/cannot_crit/force_crit 等多处使用
        effect_flags = getattr(effect, 'flags', {}) or {}
        hp_scaling_flag = effect_flags.get('hp_scaling')
        hp_scaling_value, enemy_count_scaling_bonus = self._apply_damage_compute_scaling(
            caster, effect, effect_flags, battlefield
        )
        ...
        # P0-3 Phase 1: kenki_power_tag + ignore_condition 解析独立为方法
        _ignore_def, _ignore_shld = self._apply_damage_resolve_kenki_and_ignore_cond(
            caster, effect, effect_flags, targets
        )
        ...
        dmg_skill_obj = type('obj', (object,), {
            ...
            'hp_scaling_bonus': hp_scaling_value + enemy_count_scaling_bonus,
            'cannot_crit': effect_flags.get('cannot_crit', False),
            'force_crit': effect_flags.get('force_crit', False) or self._check_force_crit_if_mark(caster, effect_flags),
            'bonus_crit_rate': 0.0,
        ...
        # 130105 PS1 整いサウナ: splash_to_adjacent enchant引用主攻击威力（「同威力」）
        self._current_main_damage_power = dmg_skill_obj.power
        ...
        # P0-3 Phase 1: conditional_power_bonus + target_hp_below_crit 独立为方法
        bonus_crit_applied = self._apply_damage_apply_conditional_power_bonus(
            caster, effect, effect_flags, targets, dmg_skill_obj
        )
        ...
        _sub_unit_buffs, _add_damage_buffs, _enchant_buffs, _carried_debuff_buffs = \
            self._prepare_enchant_buffs_for_damage_effect(caster)
        ...
        for target_idx, target in enumerate(targets):
            ...
            if target_was_dead:
                # P0-3 Phase 4: OVERKILL 分支独立为方法
                _overkill_actual = self._apply_damage_handle_overkill_target(
                    caster, target, dmg_skill_obj, is_cover_damage, battlefield,
                    targets_hit, _sub_unit_buffs, _add_damage_buffs,
                    _enchant_buffs, _carried_debuff_buffs)
                total_damage += _overkill_actual
                continue
            ...
            # P0-3 Phase 4: genwaku (幻惑) 分支独立为方法
            if self._apply_damage_handle_genwaku(
                caster, target, dmg_result, hp_before, actual_damage, battlefield,
                targets_hit, _sub_unit_buffs, _add_damage_buffs,
                _enchant_buffs, _carried_debuff_buffs):
                continue

            # P0-3 Phase 4: DMG_INVULNERABLE + freeze 独立为方法
            dmg_result, actual_damage = self._apply_damage_apply_invulnerable_and_freeze(
                target, dmg_result, actual_damage)

            # P0-3 Phase 4: 护盾+子单位吸收+最低1点伤害独立为方法
            # shield_penetration 在主方法中重新计算（方法13内部也有同样定义，但变量被封装）
            ...
            actual_damage = self._apply_damage_apply_hp_loss_and_record_hit(
                caster, target, dmg_skill_obj, dmg_result, actual_damage,
                shield_absorbed, sub_unit_absorbs, hp_before, shield_penetration,
                overflow, battlefield, targets_hit, _sub_unit_buffs, _add_damage_buffs,
                _enchant_buffs, _carried_debuff_buffs, effect, deferred_crit_actions)
            total_damage += actual_damage

            # P0-3 Phase 4: cheat_death + pending_deaths + hit_limited消耗独立为方法
            self._apply_damage_handle_post_hit_target_effects(
                target, dmg_skill_obj, hp_before, actual_damage, shield_absorbed,
                effect_flags, targets_hit)

            # attack_limited shield buff: 技能结束后统一消耗（per-skill），不再per-block消耗
            # 原per-block逻辑会导致多block技能（如追加攻撃）重复消耗shield的attack_limited，
            # 使shield过早消失。改为per-skill: 1次技能=1次攻击消耗，符合绝大部分技能表现。

        # per-target附魔处理结束后，统一消耗hit_limited（1次攻击=1次消耗，而非1个目标=1次消耗）
        self._finalize_enchant_buffs_after_damage_effect(
            caster, _add_damage_buffs, _enchant_buffs, _carried_debuff_buffs)

        # P0-3 Phase 2: 施法者 hit_limited buff 消耗独立为方法
        # 消耗其他 hit_limited buff（跳过 ENCHANT/ADD_DAMAGE/EVADE/debuff_immune/DMG_INVULNERABLE/carried_debuff/attack_limited）
        self._apply_damage_consume_caster_hit_limited_buffs(
            caster, _add_damage_buffs, _enchant_buffs, _carried_debuff_buffs)

        # P0-3 Phase 2: damage_link 转送独立为方法
        total_damage, damage_link_transfers = self._apply_damage_process_damage_links(
            caster, effect_flags, targets_hit, battlefield, total_damage)
        ...
        # P0-3 Phase 2: 延迟暴击触发器收集独立为方法
        self._apply_damage_defer_crit_triggers(deferred_crit_actions)

        # 处理 damage effect 内嵌的 lifesteal flags（如 120159 夏のせいです AS2）
        ...
        if lifesteal_pct and total_damage > 0 and caster.is_alive:
            lifesteal_effect = type('obj', (object,), {
                ...
                'value': lifesteal_pct,
                'target_type': getattr(effect, 'target_type', None),
                'flags': {},
            })()
            lifesteal_result = self._apply_lifesteal(caster, lifesteal_effect, battlefield)
    ...
    def _apply_damage_select_and_filter_targets(
        self, caster: UnitState, effect, battlefield: BattlefieldState
    ) -> Optional[List[UnitState]]:
        """选择并过滤 damage effect 的目标列表。

        涵盖原 _apply_damage L2724-L2870 逻辑：
        - 创建 target_skill_obj 用于 select_targets
        - 目标选择优先级：cached(_block_damage_targets) > debuff_applied_target(_primary_target)
          > trigger_attacker > fewest_mark_priority > 默认 select_targets
        - exclude_self 处理：debuff_applied_target 时从 _damaged_targets 中找非自身最近友方
        - 过滤：char_type_filter / element_filter
        - 扩展：expand_by_card（基于 card_buff 的 value 扩展目标数）
        - 条件过滤：hp_threshold_cross（HP穿越阈值）/ target_is_front_row / target_is_back_row
        ...
        Returns:
            目标列表；None 表示应 return None（条件过滤后无有效目标）
        """
        target_skill_obj = type('obj', (object,), {
            'display_target_type': self._resolve_target_type(effect.target_type),
            'display_target_range': self._resolve_target_range(effect.target_type),
    ...
    def _apply_damage_fire_before_attack_triggers(
        ...
        涵盖原 _apply_damage L2872-L2980 逻辑：
        - 仅当 trigger_service 可用 + 无递归保护 + _before_attack_triggers_fired=False 时执行
        - 收集 damage_targets（pre_scanned > _block_damage_targets > targets）
        - 扩展 all_block_targets（合并非伤害敌方效果目标，使触发器条件能检查全部敌方效果目标）
        - 三时机触发器调用 + simultaneous_limit 跨timing互斥（仅速度最快者触发）
        - PP不足的 simultaneous_limit PS 不参与跨timing速度竞争

        副作用：
        - 修改 self._current_attack_targets（damage-only 目标列表）
        - 修改 self._before_attack_triggers_fired（设为 True，同一技能内只触发一次）
        - 修改 self._recursion_guard（执行期间设为 True）

        Args:
            caster: 施法者
            targets: 当前 damage effect 的目标列表（用于 damage_targets 默认值）
            battlefield: 战场状态
        """
        if self.trigger_service and not self._recursion_guard and not self._before_attack_triggers_fired:
            ...
            try:
                ...
                if pre_scanned:
                    damage_targets = [u for u in pre_scanned if u.is_alive]
                else:
                    damage_targets = targets  # 默认使用当前damage效果的目标
                    ...
                    if bdt and isinstance(bdt, dict):
                        ...
                        for units in bdt.values():
                            for u in units:
                                if u.unit_id not in seen_ids and u.is_alive:
                                    seen_ids.add(u.unit_id)
                                    damage_targets.append(u)
                        if not damage_targets:
                            damage_targets = targets
                # cover候选保持damage-only（cover只保护被伤害的友方）
                self._current_attack_targets = list(damage_targets)
                # 伤害+效果技能：将非伤害敌方效果目标（如spd_down的enemy_all目标）合并到
                # all_block_targets，使before_any_attacked/before_as_attacked等触发器的
                # target_is_self/target_is_front_ally等条件能检查到全部敌方效果目标。
                # 纯效果技能（无damage）不会执行到此分支，因此不会误触发guard/evasion PS。
                # cover PS使用_pre_scanned_cover_candidates（damage-only）选择cover目标，
                # 不受all_block_targets扩展影响。
                all_block_targets = list(damage_targets)
                _nd_targets = getattr(self, '_pre_scanned_non_damage_enemy_targets', [])
                if _nd_targets:
                    _seen_bt_ids = set(u.unit_id for u in all_block_targets)
                    for _nd_u in _nd_targets:
                        if _nd_u.unit_id not in _seen_bt_ids and _nd_u.is_alive:
                            _seen_bt_ids.add(_nd_u.unit_id)
                            all_block_targets.append(_nd_u)
                    if len(all_block_targets) > len(damage_targets):
                        _log.info("[TRIGGER_TARGETS] %s: expanded all_block_targets from %d to %d (incl non-damage enemy targets: %s)",
                                  caster.name, len(damage_targets), len(all_block_targets),
                                  [u.name for u in all_block_targets if u.unit_id not in set(x.unit_id for x in damage_targets)])
    ...
    def _apply_damage_handle_overkill_target(
        self, caster: UnitState, target: UnitState, dmg_skill_obj,
        is_cover_damage: bool, battlefield: BattlefieldState, targets_hit: list,
        sub_unit_buffs: list, add_damage_buffs: list,
        enchant_buffs: list, carried_debuff_buffs: list) -> int:
        """处理 OVERKILL 分支（目标已死亡时直接计算伤害并记录）。

        涵盖原 _apply_damage L2875-L2922 逻辑：
        - 前置条件: target.is_alive == False (由调用方判断 target_was_dead)
        - 计算伤害（目标HP=0，全为溢出伤害）
        - 累计到 _block_damage_total / caster.damage_dealt_total / target.damage_taken_total
        - 记录到 targets_hit (overkill=True)
        - 调用 scoring_tracker.record_damage (overflow=actual_damage for enemy)
        - 调用 _process_enchant_damage_for_target (OVERKILL路径: is_overkill=True 跳过 sub_unit/add_damage/enchant)

        Args:
            caster: 施法者
            target: 已死亡的目标单位
            dmg_skill_obj: 伤害计算参数对象
            is_cover_damage: 是否为cover替换位置的伤害
            battlefield: 战场状态
            targets_hit: 已命中目标列表（方法会 append 新元素）
            sub_unit_buffs/add_damage_buffs/enchant_buffs/carried_debuff_buffs: per-target附魔处理所需buff列表

        Returns:
            actual_damage: OVERKILL 伤害值（供调用方累加 total_damage）
        """
        dmg_result = self.damage_service.calculate_damage(
            caster, target, dmg_skill_obj, is_cover_damage=is_cover_damage,
            on_crit_callback=self._make_on_crit_callback(caster, battlefield))
        actual_damage = dmg_result.total_damage
        target.current_hp = 0
        ...
        # 计分追踪：记录OVERKILL伤害（目标已倒下，全部为溢出伤害）
        _scoring_tracker = getattr(battlefield, 'scoring_tracker', None)
        if _scoring_tracker is not None:
            _caster_side = "ally" if caster.side.value == "ally" else "enemy"
            _target_side = "ally" if target.side.value == "ally" else "enemy"
            _scoring_tracker.record_damage(
                source_id=caster.unit_id, source_name=caster.name, source_side=_caster_side,
                target_id=target.unit_id, target_name=target.name, target_side=_target_side,
                actual_damage=actual_damage, shield_absorbed=0,
                overflow=actual_damage if _target_side == "enemy" else 0,
            )

        _log.info("[DAMAGE_APPLY] %s -> %s (OVERKILL): hp 0→0 (-%d) crit=%s",
                  caster.name, target.name, actual_damage, dmg_result.is_critical)
        # per-target附魔处理（OVERKILL路径：is_overkill=True时跳过sub_unit/add_damage/enchant；
        # carried_debuff使用自身is_alive检查，OVERKILL目标已死亡故也会跳过）
        self._process_enchant_damage_for_target(
            caster, targets_hit[-1], battlefield,
            sub_unit_buffs, add_damage_buffs,
            enchant_buffs, carried_debuff_buffs)
        return actual_damage

    def _apply_damage_handle_genwaku(
        self, caster: UnitState, target: UnitState, dmg_result,
        hp_before: int, actual_damage: int, battlefield: BattlefieldState,
        targets_hit: list, sub_unit_buffs: list, add_damage_buffs: list,
        enchant_buffs: list, carried_debuff_buffs: list) -> bool:
        """处理幻惑(genwaku)分支：攻击者持有幻惑时，伤害转为回复。

        涵盖原 _apply_damage L2890-L2931 逻辑：
        - 检查 caster.debuffs 是否有 GENWAKU buff
        - 回复值 = 最终伤害值 × genwaku%，受目标回复量修正影响
        - per-hit独立计算回复值，避免比例分配时余数累积到最后一hit
        - 实际回复受 target_max_hp - target.current_hp 上限
        - 记录到 targets_hit (genwaku_heal/genwaku_hit_heals 字段)
        - 调用 _process_enchant_damage_for_target (genwaku路径: 主伤害转回复，附魔仍按原逻辑处理)

        Args:
            caster: 施法者（持有 GENWAKU debuff）
            target: 目标单位
            dmg_result: 主攻击伤害计算结果
            hp_before: 目标伤害前HP
            actual_damage: 主攻击伤害值（用于日志展示）
            battlefield: 战场状态
            targets_hit: 已命中目标列表（方法会 append 新元素）
            sub_unit_buffs/add_damage_buffs/enchant_buffs/carried_debuff_buffs: per-target附魔处理所需buff列表

        Returns:
            True: 已处理 genwaku（持有 GENWAKU buff），主循环应 continue
            False: 无 GENWAKU buff，主循环继续正常路径
        """
        genwaku_buffs = [b for b in caster.debuffs if b.effect_type == SkillEffectType.GENWAKU.value]
        if not genwaku_buffs:
            return False
        ...
        # per-target附魔处理（genwaku路径：主伤害转为回复，附魔/sub_unit/add_damage仍按原逻辑处理）
        self._process_enchant_damage_for_target(
            caster, targets_hit[-1], battlefield,
            sub_unit_buffs, add_damage_buffs,
            enchant_buffs, carried_debuff_buffs)
    ...
    def _apply_damage_apply_invulnerable_and_freeze(
        self, target: UnitState, dmg_result, actual_damage: int
    ) -> Tuple[Any, int]:
        """处理 DMG_INVULNERABLE 伤害无效化 + freeze 受击增伤解除。

        涵盖原 _apply_damage L2896-L2979 逻辑：

        DMG_INVULNERABLE (伤害无效化):
        - threshold_pct > 0: 現在HPのX%を超える攻撃のみダメージを無効にする
        - threshold_pct = 0: 全ての攻撃ダメージを無効にする（攻撃無効化）
        - 每hit独立判断: hit_dmg > threshold_value 时无效化为1点伤害
        - hit_limited 递减: 每次 nullify 消耗1次，到0时移除buff（含 linked HOT 同步移除）
        - 修改 dmg_result (重建 hit_details/hit_crits/hit_evades) 和 actual_damage

        FREEZE (冻结受击增伤):
        - 目标有 FREEZE debuff 且非完全闪避时: 伤害增加 freeze.value%，解除冻结
        - 完全闪避时: 不解除冻结（_log.info [FREEZE_KEEP]）

        Args:
            target: 目标单位（持有 DMG_INVULNERABLE buff 或 FREEZE debuff）
            dmg_result: 主攻击伤害计算结果
            actual_damage: 当前伤害值

        Returns:
            (dmg_result, actual_damage): 可能被修改的伤害结果和伤害值
        """
        ...
        return dmg_result, actual_damage

    def _apply_damage_absorb_shields_and_subunits(
        self, caster: UnitState, target: UnitState, dmg_skill_obj,
        dmg_result, actual_damage: int, battlefield: BattlefieldState
    ) -> Tuple[int, int, list]:
        """处理护盾吸收 + 子单位吸收 + 最低1点伤害。

        涵盖原 _apply_damage L2901-L3026 逻辑：

        护盾吸收顺序（shield_penetration < 1.0 时）:
        1. EN护盾 (en_shield): 仅 EN 类型攻击 (caster_char_type == 2)
        2. 物理护盾 (physical_shield): 仅非 EN 类型攻击
        3. 通用护盾 (shield): 任意攻击
        4. 子单位 (SUB_UNIT buff): 任意攻击，按 buffs 列表顺序

        最低1点伤害规则 (actual_damage <= 0 and dmg_result.total_damage > 0):
        - 非闪避命中最低1点伤害，可作用于护盾或HP
        - 按相同顺序（盾→子单位→HP）尝试吸收1点
        - 都未吸收时 actual_damage = 1

        Args:
            caster: 施法者（用于 character_type 判断 EN/物理）
            target: 目标单位（持有 shields/sub_units）
            dmg_skill_obj: 伤害计算参数对象（含 ignore_shield）
            dmg_result: 主攻击伤害计算结果（用于 total_damage 判断最低1点）
            actual_damage: 当前伤害值
            battlefield: 战场状态（用于子单位 cascade 联动）

        Returns:
            (actual_damage, shield_absorbed, sub_unit_absorbs):
            - actual_damage: 吸收后的最终伤害值（≥0，最低1点时=1）
            - shield_absorbed: 护盾吸收的总量
            - sub_unit_absorbs: 子单位吸收信息列表（供叙事日志使用）
        """
        shield_absorbed = 0
        shield_penetration = 0
        if dmg_skill_obj.ignore_shield:
        ...
        return actual_damage, shield_absorbed, sub_unit_absorbs

    def _apply_damage_apply_hp_loss_and_record_hit(
        self, caster: UnitState, target: UnitState, dmg_skill_obj, dmg_result,
        actual_damage: int, shield_absorbed: int, sub_unit_absorbs: list,
        hp_before: int, shield_penetration: float, overflow: int,
        battlefield: BattlefieldState, targets_hit: list,
        _sub_unit_buffs: list, _add_damage_buffs: list,
        _enchant_buffs: list, _carried_debuff_buffs: list,
        effect, deferred_crit_actions: list
    ) -> int:
        """HP扣减 + 累计伤害 + per-hit追踪 + targets_hit收集 + 附魔 + 计分 + 暴击延迟。

        涵盖原 _apply_damage L2903-L3013 逻辑：

        1. HP扣减: target.current_hp = max(0, target.current_hp - actual_damage)
        2. 累计伤害计数: target.cumulative_hp_damage += hp_loss (仅HP部分，不含盾吸收)
        3. last_received_damage: 累计当次攻击所有hit的伤害（含盾吸收，不含溢出），
           用于反撃系PS（如ストイックリコイル）
        4. block_damage_total: 累计到 _block_damage_total[current_block_id] (供 split_heal_by_damage 读取)
        5. per-hit HP损失追踪: 计算各hit的HP损失，追加到 _per_hit_hp_losses[target.unit_id]
           - 補正: 盾吸收/最低1点伤害等导致合计与hp_loss不一致时，最后hit调整
        6. targets_hit.append: 收集目标命中信息（hp_before/after, damage, crit, hits, etc.）
        7. per-target附魔处理: _process_enchant_damage_for_target（force_crit基于主攻击hit_crits）
        8. 全段闪避追踪: dmg_result.hit_evades 全True时，加入 _block_evaded_targets 和 _skill_evaded_targets
        9. damage_totals: caster.damage_dealt_total += actual_damage; target.damage_taken_total += actual_damage
        10. scoring_tracker: 记录伤害（actual_damage含溢出全部计入得分，己方目标overflow=0）
        11. rdps_tracker: RDPS归因（主伤害）
        12. 暴击延迟触发器: dmg_result.is_critical 时追加到 deferred_crit_actions
            - crit_hit_count = sum(1 for c in hit_crits if c) 或 1
        13. on_crit_blocks延迟: dmg_result.is_critical 且 _on_crit_blocks 非空时，
            追加到 _deferred_on_crit_targets（避免on_crit块效果被后续block错误清除）
        14. 日志输出: [DAMAGE_APPLY] hp before→after (-damage) [shield=N] 💀DEAD crit=X

        Args:
            caster: 施法者
            target: 目标单位
            dmg_skill_obj: 伤害计算参数对象（含 hit_count）
            dmg_result: 伤害计算结果（含 total_damage/hit_details/hit_crits/hit_evades/is_critical/calc_detail）
            actual_damage: 当前伤害值（吸收后）
            shield_absorbed: 护盾吸收的总量
            sub_unit_absorbs: 子单位吸收信息列表（供叙事日志使用）
            hp_before: 目标受击前HP
            shield_penetration: 盾穿透比例 [0, 1]
            overflow: 溢出伤害 = max(0, actual_damage - hp_before)
            battlefield: 战场状态（用于 scoring_tracker / rdps_tracker / 附魔处理）
            targets_hit: 目标命中列表（按引用修改，追加新元素）
            _sub_unit_buffs: 子单位buff列表（附魔处理用）
            _add_damage_buffs: 追加伤害buff列表（附魔处理用）
            _enchant_buffs: 附魔伤害buff列表（附魔处理用）
            _carried_debuff_buffs: 携带debuff列表（附魔处理用）
            effect: 技能效果对象（on_crit_blocks延迟时记录到 _deferred_on_crit_targets）
            deferred_crit_actions: 延迟暴击触发器列表（按引用修改）

        Returns:
            actual_damage: 该目标本次受到的伤害值（供主方法累加到 total_damage）
        """
        # HP扣减
        ...
        if hp_loss > 0:
            ...
            # 130158 ヒートアップ・ラブ: 累计当次行动中受到的伤害（供 damage_taken_in_action_percent 检查）
        ...
        # per-target附魔处理（正常路径：force_crit基于主攻击hit_crits，与主伤害同步结算）
        self._process_enchant_damage_for_target(
            caster, targets_hit[-1], battlefield,
            _sub_unit_buffs, _add_damage_buffs,
            _enchant_buffs, _carried_debuff_buffs)
    ...
    def _apply_damage_handle_post_hit_target_effects(
        self, target: UnitState, dmg_skill_obj, hp_before: int,
        actual_damage: int, shield_absorbed: int,
        effect_flags: dict, targets_hit: list
    ) -> None:
        """处理受击后目标侧效果：cheat_death + pending_deaths + hit_limited debuff消耗 + shield hit_limited消耗。

        涵盖原 _apply_damage L3015-L3065 逻辑：

        1. cheat_death: target.current_hp <= 0 时检查 cheat_death buff
           - 有 cheat_death: 回复到 max_hp * heal_pct / 100 (至少1)
             - 多段cheat_death (stack_count>1): 递减 stack_count 而非移除
             - 单段: 移除 cheat_death buff
           - 无 cheat_death: 加入 _pending_deaths (延迟阵亡判定)
        2. hit_limited debuff消耗: per-hit 递减（dmg_skill_obj.hit_count 次）
           - 跳过 attack_limited>0 的debuff（由技能结束时的attack_limited清理统一处理）
           - 跳过 DEALT_DAMAGE 类型（在 damage_service 中 per-hit 消耗并重算dealt_mult）
           - hit_limited 减到0时移除该debuff
        3. shield hit_limited消耗: shield_absorbed > 0 且未设置 skip_attack_limited_shield 时
           - 找到 shield 类型且 hit_limited>0 的 buff
           - 移除剩余护盾值（target.shield=0），设置 hit_limited=0，移除buff
           - 记录 shield_expired 信息到 targets_hit[-1]

        Args:
            target: 受击目标
            dmg_skill_obj: 伤害计算参数对象（含 hit_count）
            hp_before: 受击前HP（用于日志，当前未使用，保留接口）
            actual_damage: 本次伤害值（用于日志，当前未使用，保留接口）
            shield_absorbed: 护盾吸收总量
            effect_flags: 效果标志字典（检查 skip_attack_limited_shield）
            targets_hit: 目标命中列表（记录 shield_expired 信息）

        Returns:
            None（所有修改都作用于 target / targets_hit / self._pending_deaths）
        """
        # cheat_death 处理
    ...
    def _apply_damage_compute_scaling(
        self, caster: UnitState, effect, effect_flags: dict, battlefield: BattlefieldState
    ) -> Tuple[float, float]:
        """计算 hp_scaling 和 enemy_count_scaling 增伤加成。

        涵盖原 _apply_damage L2997-L3067 逻辑：
        - hp_scaling: 基于施法者/敌方HP比例的增伤（支持 max_tag 动态解析、hp_scaling_enemy、hp_scaling_inverse）
        - dmg_scale_by_enemy_count: 生存敌方数比例增伤（110004 空想造形アーティスト）

        Args:
            caster: 施法者
            effect: 技能效果对象
            effect_flags: effect.flags 字典
            battlefield: 战场状态

        Returns:
            (hp_scaling_value, enemy_count_scaling_bonus) 二元组
        """
        hp_scaling_flag = effect_flags.get('hp_scaling')
        ...
        # dmg_scale_by_enemy_count: 生存中の敵数比例でダメージ増加(110004 空想造形アーティスト)
        # 公式: bonus_pct = max_pct × alive_enemies / initial_enemy_count
        # 例: max_pct=100, 6敵中6生存→+100%, 6敵中3生存→+50%, 1敵中1生存→+100%
        # hp_scaling_bonusと同じ乗区(造成伤害bonus)に加算
        enemy_count_scaling_bonus = 0.0
        _dmg_scale_ec = effect_flags.get('dmg_scale_by_enemy_count')
        if _dmg_scale_ec and isinstance(_dmg_scale_ec, dict):
            _ec_max_pct = float(_dmg_scale_ec.get('max_pct', 0))
            if _ec_max_pct > 0:
                from src.entities_v2.enums import Side as _SideECS
                _opposing_team = (battlefield.enemy_team if caster.side == _SideECS.ALLY
                                  else battlefield.friend_team)
                _alive_enemies = sum(1 for u in _opposing_team if u.is_alive)
                _initial_count = len(_opposing_team)
                if _initial_count > 0:
                    _ec_ratio = _alive_enemies / _initial_count
                    enemy_count_scaling_bonus = _ec_max_pct * _ec_ratio
                    _log.info("[DAMAGE_APPLY] %s: dmg_scale_by_enemy_count alive=%d initial=%d ratio=%.3f bonus=%.1f%%",
                              caster.name, _alive_enemies, _initial_count, _ec_ratio, enemy_count_scaling_bonus)

        return hp_scaling_value, enemy_count_scaling_bonus

    def _apply_damage_resolve_kenki_and_ignore_cond(
        self, caster: UnitState, effect, effect_flags: dict, targets: List[UnitState]
    ) -> Tuple[int, int]:
        """解析 kenki_power_tag 切换和条件性穿防穿盾。

        涵盖原 _apply_damage L3078-L3131 逻辑：
        - kenki_power_tag: 行动开始时有「剣気」mark 时切换 value_tag 并重新解析 effect.value
        - ignore_condition: 条件不满足时禁用穿防穿盾（target_has_burn/status_ailment/poison）

        副作用：当 kenki_power_tag 触发时，修改 effect.value_tag 和 effect.value

        Args:
            caster: 施法者
            effect: 技能效果对象（可能被修改 value_tag/value）
            effect_flags: effect.flags 字典
            targets: 目标单位列表（用于 ignore_condition 检查）

        Returns:
            (_ignore_def, _ignore_shld) 二元组：实际使用的穿防/穿盾值
        """
        ...
        return _ignore_def, _ignore_shld

    def _apply_damage_apply_conditional_power_bonus(
        self, caster: UnitState, effect, effect_flags: dict, targets: List[UnitState], dmg_skill_obj
    ) -> float:
        """应用 conditional_power_bonus 条件增伤和 target_hp_below_crit 暴击加成。

        涵盖原 _apply_damage L3163-L3349 逻辑：
        - 6种条件类型：target_hp_below / target_has_status_ailment / target_has_burn /
          target_has_debuff / target_has_mark / target_mark_count
        - 2种 mark_count 模式：tiers 阶梯匹配 / multiplier_per_count 线性缩放
        - 3种 bonus_type：power(修改skill power) / dealt_damage(临时dmg_dealt_up buff) /
          received_damage(临时dmg_taken_up debuff)
        - target_hp_below_crit: 目标HP低于阈值时给 dmg_skill_obj.bonus_crit_rate 加值

        副作用：
        - 修改 dmg_skill_obj.power（bonus_type=power 时）
        - 修改 dmg_skill_obj.bonus_crit_rate（target_hp_below_crit 触发时）
        - 添加临时 buff 到 caster.buffs（dealt_damage 时）
        - 添加临时 debuff 到 targets[0].debuffs（received_damage 时）
        ...
        Args:
            ...
            effect_flags: effect.flags 字典
            targets: 目标单位列表
            dmg_skill_obj: 伤害计算技能对象（可能被修改 power/bonus_crit_rate）

        Returns:
            bonus_crit_applied: 实际应用的暴击加成值
        """
        ...
        if cond_power_bonus and isinstance(cond_power_bonus, dict) and targets:
            ...
            if cond_type == 'target_hp_below' and targets:
                ...
                # 多体攻撃時: 全目標中の最大mark数を採用（いずれかの目標がmarkを持てば增伤発動）
                mark_name = cond.get('mark_name', '')
                # 全目標から最大mark数を取得（单体攻撃時はtargets[0]のみ）
                max_target_mark_count = 0
                for _tgt in targets:
                    _cnt = sum(1 for d in _tgt.debuffs
                               if d.effect_type == SkillEffectType.MARK.value and getattr(d, 'name', '') == mark_name)
                    _cnt += sum(1 for b in _tgt.buffs
                                if b.effect_type == SkillEffectType.MARK.value and getattr(b, 'name', '') == mark_name)
                    if _cnt > max_target_mark_count:
                        max_target_mark_count = _cnt
                mark_count = max_target_mark_count
            ...
            if cond_met:
                ...
                # 优先使用配置中的固定 value（如120119 警棒+15%增伤，全等级固定值无需从skills.json读取）
                # 通过resolver解析tag值
                bonus_pct = 50.0  # 默认值
                if cond_tier_value is not None:
                    # target_mark_count tiers: bonus_pct已在条件检查时从matched_tier获取
                    bonus_pct = cond_tier_value
                elif 'value' in cond_power_bonus:
                    # 直接配置固定值（如120119 警棒+15%增伤，全等级固定值无需从skills.json读取）
                    bonus_pct = float(cond_power_bonus['value'])
                elif value_tag and hasattr(self, '_resolver') and self._resolver:
                    _skill_level = caster.skill_levels.get(self._current_skill_id, 1)
                    meta = self.data_loader.get_skill_by_id(self._current_skill_id)
                    if meta:
                        tag_values = self._resolver._resolve_template_tags(meta, _skill_level)
        ...
        return bonus_crit_applied

    def _apply_damage_consume_caster_hit_limited_buffs(
        self, caster: UnitState, _add_damage_buffs: list,
        _enchant_buffs: list, _carried_debuff_buffs: list) -> None:
        """消耗施法者的 hit_limited buff（1次攻击=1次消耗）。

        涵盖原 _apply_damage L3899-L3918 逻辑：
        - 遍历 caster.buffs，对 hit_limited > 0 的 buff 递减 1
        - 跳过 ENCHANT_DAMAGE / ADD_DAMAGE（由 _finalize_enchant_buffs_after_damage_effect 消耗）
        - 跳过 EVADE（由 damage_service 闪避时消耗）
        - 跳过 debuff_immune（由 _consume_debuff_immune 阻挡 debuff 时消耗）
        - 跳过 DMG_INVULNERABLE（由伤害无效化逻辑消耗）
        - 跳过 carried_debuff（由 _process_enchant_damage_for_target 消耗）
        - 跳过 attack_limited > 0（由技能结束时统一消耗）
        - hit_limited 递减到 ≤0 时从 caster.buffs 移除

        Args:
            caster: 施法者
            _add_damage_buffs: 预收集的 ADD_DAMAGE buff 列表（保留参数，本方法不用）
            _enchant_buffs: 预收集的 ENCHANT_DAMAGE buff 列表（保留参数，本方法不用）
            _carried_debuff_buffs: 预收集的 carried_debuff buff 列表（保留参数，本方法不用）
        """
        for b in list(caster.buffs):
            if b.hit_limited > 0:
                if b.effect_type == SkillEffectType.ENCHANT_DAMAGE.value:
                    continue
                if b.effect_type == SkillEffectType.ADD_DAMAGE.value:
                    continue
                ...
                if b.hit_limited <= 0:
                    ...
                    _log.info("[HIT_LIMITED] %s: caster buff %s EXPIRED (hit_limited reached 0)",
                              caster.name, b.effect_type)
    ...
    def _apply_damage_process_damage_links(
        ...
        Args:
            caster: 施法者
            effect_flags: effect.flags 字典
            targets_hit: 每个目标的伤害信息列表（会被修改：hp_after/actual_damage/hits 等）
            battlefield: 战场状态
            total_damage: 当前累计伤害值

        Returns:
            (new_total_damage, damage_link_transfers) 二元组
        """
                ...
                for dl in active_links:
                    ...
                    if linker and linker.is_alive and linker.unit_id != target_unit.unit_id:
                        ...
                        if link_hp_loss > 0:
                            ...
                            linker._damage_taken_in_action = getattr(linker, '_damage_taken_in_action', 0) + link_hp_loss
                        # 计分追踪
                        _scoring_tracker = getattr(battlefield, 'scoring_tracker', None)
                        if _scoring_tracker is not None:
    ...
    def _apply_damage_process_reflect(
        self, caster: UnitState, targets_hit: list,
        battlefield: BattlefieldState) -> list:
        """处理 reflect_damage 反射（cover 替换位置的伤害）。

        涵盖原 _apply_damage L4053-L4106 逻辑：
        - 仅 _cover_replaced_indices 中的 target 才反射（cover 替换位置）
        - 反射伤害 = cover者承受的总伤害（HP损失+盾吸收+子单位吸收）× reflect_rate%
        - 反射伤害不被盾吸收（直接扣 caster HP），不享受减伤
        - 反射伤害不再被反射（无递归）
        - 更新 caster.damage_taken_total / coverer.damage_dealt_total
        - 计分追踪（scoring_tracker）

        Args:
            caster: 施法者（被反射的目标）
            targets_hit: 每个目标的伤害信息列表
            battlefield: 战场状态

        Returns:
            reflect_transfers: 反射伤害信息列表，供叙事日志输出
        """
        reflect_transfers = []
        cover_indices = getattr(self, '_cover_replaced_indices', set())
        if cover_indices and caster.is_alive:
            for target_idx in cover_indices:
                if target_idx >= len(targets_hit):
                    continue
                tgt_info = targets_hit[target_idx]
                coverer_id = tgt_info.get("target_id")
                coverer = next((u for u in battlefield.get_all_units() if u.unit_id == coverer_id), None)
                if not coverer or not coverer.is_alive or coverer.reflect_rate <= 0:
                    continue
                # cover者承受的总伤害 = HP损失 + 盾/子单位吸收（与dmg_result.total_damage一致，不含溢出）
                source_total_dmg = tgt_info.get("actual_damage", 0) + tgt_info.get("shield_absorbed", 0)
                if source_total_dmg <= 0:
                    continue
                reflect_dmg = int(source_total_dmg * coverer.reflect_rate / 100)
                if reflect_dmg <= 0:
                    continue
                caster_hp_before = caster.current_hp
                # 反射伤害不被盾吸收（直接扣HP），也不享受减伤（简化处理）
                caster.current_hp = max(0, caster.current_hp - reflect_dmg)
                caster.damage_taken_total += reflect_dmg
                coverer.damage_dealt_total += reflect_dmg
                _log.info("[REFLECT_DAMAGE] %s -> %s: reflected %d dmg (%.0f%% of %d [hp:%d+shield:%d]), caster hp %d->%d",
                          coverer.name, caster.name, reflect_dmg, coverer.reflect_rate,
                          source_total_dmg, tgt_info.get("actual_damage", 0), tgt_info.get("shield_absorbed", 0),
                          caster_hp_before, caster.current_hp)
                # 计分追踪
                _scoring_tracker = getattr(battlefield, 'scoring_tracker', None)
                if _scoring_tracker is not None:
                    _coverer_side = "ally" if coverer.side.value == "ally" else "enemy"
                    _caster_side = "ally" if caster.side.value == "ally" else "enemy"
                    _scoring_tracker.record_damage(
                        source_id=coverer.unit_id, source_name=coverer.name, source_side=_coverer_side,
                        target_id=caster.unit_id, target_name=caster.name, target_side=_caster_side,
                        actual_damage=reflect_dmg, shield_absorbed=0,
                        overflow=reflect_dmg if _caster_side == "enemy" else 0,
                    )
                reflect_transfers.append({
                    "coverer_id": coverer.unit_id,
                    "coverer_name": coverer.name,
                    "attacker_id": caster.unit_id,
                    "attacker_name": caster.name,
                    "reflect_dmg": reflect_dmg,
                    "hp_before": caster_hp_before,
                    "hp_after": caster.current_hp,
                    "max_hp": caster.max_hp,
                    "reflect_rate": coverer.reflect_rate,
                    "source_total_damage": source_total_dmg,
                })

        return reflect_transfers

    def _apply_damage_defer_crit_triggers(self, deferred_crit_actions: list) -> None:
        """收集延迟暴击触发器到 _pending_crit_triggers（在 execute_skill 末尾统一触发）。

        涵盖原 _apply_damage L4115-L4135 逻辑：
        - 仅当 deferred_crit_actions 非空 + 有 trigger_service + 无递归保护时触发
        - 同一 caster 的 crit_count 累加（每技能仅触发一次 PS，但 crit_counter 按 hit 数累加）
        - crit_target 取最新暴击目标（多目标暴击时取最后的目标）

        Args:
            deferred_crit_actions: 延迟暴击触发器列表，每项为 (caster, battlefield, crit_count, crit_target)
        """
    ...
    def _prepare_enchant_buffs_for_damage_effect(self, caster: UnitState) -> tuple:
        """Collect enchant-related buff lists and initialize per-target cache.

        Returns (sub_unit_buffs, add_damage_buffs, enchant_buffs, carried_debuff_buffs).
        Initializes self._per_target_enchant_cache to a dict if any buffs exist, else None.
        """
        enchant_buffs = [b for b in caster.buffs if b.effect_type == SkillEffectType.ENCHANT_DAMAGE.value]
        add_damage_buffs = [b for b in caster.buffs if b.effect_type == SkillEffectType.ADD_DAMAGE.value]
        ...
        carried_debuff_buffs = [b for b in caster.buffs
                                if getattr(b, 'hit_limited_flags', {}).get('carried_debuff')]
        if not enchant_buffs and not add_damage_buffs and not carried_debuff_buffs and not sub_unit_buffs:
            self._per_target_enchant_cache = None
            return sub_unit_buffs, add_damage_buffs, enchant_buffs, carried_debuff_buffs

        _log.info("[ENCHANT_DMG] %s: prepared %d enchant_damage + %d add_damage + %d carried_debuff + %d sub_unit buffs",
                  caster.name, len(enchant_buffs), len(add_damage_buffs), len(carried_debuff_buffs), len(sub_unit_buffs))
        ...
        self._per_target_enchant_cache = {
            "enchant_targets": [],
            "splash_targets": [],
    ...
    def _process_enchant_damage_for_target(self, caster: UnitState, target_info: dict,
                                            ...
                                            sub_unit_buffs: list, add_damage_buffs: list,
                                            enchant_buffs: list, carried_debuff_buffs: list) -> int:
        """Process enchant-related damage for a single target.

        Crit is shared from main attack: force_crit = any(target_info.get("hit_crits", [])).
        Appends results to self._per_target_enchant_cache.
        Returns total damage delta for this target.
        """
        # 若无任何附魔类buff，_prepare_enchant_buffs_for_damage_effect 将 cache 设为 None，跳过处理
        if self._per_target_enchant_cache is None:
            return 0
        target = next((u for u in battlefield.get_all_units() if u.unit_id == target_info.get("target_id")), None)
        if not target:
            return 0

        # 暴击共享: 主攻击n hit中任意1hit暴击，附魔/子单位/追加伤害固定为暴击
        force_crit = any(target_info.get("hit_crits", []))

        target_evades = target_info.get("hit_evades", [])
        fully_evaded = bool(target_evades) and all(target_evades)
        is_overkill = (not target.is_alive) and (target_info.get("hp_before", 0) <= 0)
        # sub_unit/add_damage/enchant skip on full-evade or OVERKILL; carried_debuff uses own check
        skip_damage_buffs = fully_evaded or is_overkill
        ...
        # _splash_target_objects: 隣接ターゲットのUnitStateオブジェクトを収集
        # (carried_debuffをsplash対象にも付与するため、cacheのIDだけでなく実体も保持)
        _splash_target_objects = []
        for eb in enchant_buffs:
            _eb_hlf_splash = getattr(eb, 'hit_limited_flags', {}) or {}
            if not _eb_hlf_splash.get('splash_to_adjacent'):
                continue
            # skip_damage_buffs covers fully_evaded and is_overkill
            if skip_damage_buffs:
                _log.info("[SPLASH_DMG] %s: skipping splash for %s (fully_evaded=%s is_overkill=%s)",
                          caster.name, target.name, fully_evaded, is_overkill)
                continue
            # 主攻撃の威力を参照（「同威力」）
            _main_power = self._current_main_damage_power
            if _main_power is None or _main_power <= 0:
                _log.info("[SPLASH_DMG] %s: no _current_main_damage_power, skip splash", caster.name)
                continue
            # 隣接する敵を取得（主目標と同サイドの隣接存活单位、主目標自身は除く）
            if not self.target_service:
                continue
            _adjacent_enemies = self.target_service.get_adjacent_to_unit(target, battlefield, caster)
            if not _adjacent_enemies:
                _log.info("[SPLASH_DMG] %s: no adjacent enemies for %s, skip splash",
                          caster.name, target.name)
                continue
            _source_unit_id_splash = eb.source_unit_id
            # splash damge計算用のdmg_skill_obj（メイン攻撃と同じ威力）
            _splash_skill_obj = type('obj', (object,), {
                'power': _main_power,
                'hit_count': 1,
                'element': caster.element,
                'ignore_defense': 0,
            ...
            for _adj_target in _adjacent_enemies:
                if not _adj_target.is_alive:
                    continue
                _adj_hp_before = _adj_target.current_hp
                _adj_dmg_result = self.damage_service.calculate_damage(
                ...
                if _adj_hp_loss > 0:
                    _adj_target.cumulative_hp_damage += _adj_hp_loss
                    # 130158 ヒートアップ・ラブ: 累计当次行动中受到的伤害
                ...
                # 計分追跡
                _tracker = getattr(battlefield, 'scoring_tracker', None)
                if _tracker is not None:
                    _src_side = "ally" if caster.side.value == "ally" else "enemy"
                    _tgt_side = "ally" if _adj_target.side.value == "ally" else "enemy"
                    _tracker.record_damage(
                        source_id=caster.unit_id, source_name=caster.name, source_side=_src_side,
                        target_id=_adj_target.unit_id, target_name=_adj_target.name, target_side=_tgt_side,
                        actual_damage=_adj_actual_dmg, shield_absorbed=_adj_shield_absorbed,
                    )
                # RDPS帰属
                _rdps_tracker = getattr(battlefield, 'rdps_tracker', None)
                if _rdps_tracker is not None and _adj_actual_dmg > 0:
                    _rdps_tracker.record_damage_with_attribution(
                        caster=caster, target=_adj_target, actual_damage=_adj_actual_dmg,
                        damage_type="enchant",
                        enchant_source_id=_source_unit_id_splash,
                        battlefield=battlefield, damage_service=self.damage_service,
                        enchant_buff=eb,
                    )
                self._per_target_enchant_cache["splash_targets"].append({
                    "target_id": _adj_target.unit_id,
                    "target": _adj_target.unit_id,
                    "hp_before": _adj_hp_before,
                    "hp_after": _adj_target.current_hp,
                    "actual_damage": _adj_actual_dmg,
                    "damage": _adj_actual_dmg,
                    "crit": _adj_dmg_result.is_critical,
                    "hits": _adj_dmg_result.hit_details,
                    "hit_crits": _adj_dmg_result.hit_crits,
                    "modifiers": ["隣接"],
                    "calc_detail": _adj_dmg_result.calc_detail,
                })
                _log.info("[SPLASH_DMG] %s splash -> %s: dmg=%d (power=%.1f crit=%s) hp: %d->%d",
                          caster.name, _adj_target.name, _adj_actual_dmg, _main_power,
                          _adj_dmg_result.is_critical, _adj_hp_before, _adj_target.current_hp)
                # スキル対象リストに追加（dmg_dealt_down等のcarried_debuff対象とするため）
                self._skill_all_attacked_targets.append(_adj_target)
                # carried_debuff付与対象として収集
                _splash_target_objects.append(_adj_target)
            # splash enchant buffのhit_limitedを消費
            if eb.hit_limited > 0:
                eb.hit_limited -= 1

        # 携带式debuff载荷：支持多种debuff类型(atk_down/def_down/spd_down/crit_rate_down等)
        # 显示标签映射: effect_type -> (stat_label, short_name)
        # 130105 PS1: 対群攻撃(splash_to_adjacent)の場合、carried_debuffは主目標+隣接目標の
        # 全てに付与される（スキル対象となった全ての敵 = main + splash）
        _CARRIED_DEBUFF_LABELS = {
            ...
            'crit_rate_down': 'CRT',
            'poison': 'POI',
            'heal_efficacy_down': 'HEAL',
            'dmg_dealt_down': 'DEALT',
        }
        # carried_debuff付与対象のリストを構築: 主目標 + splash目標
        # (splash_to_adjacent enchantで収集した_splash_target_objectsを含める)
        _carried_debuff_target_list = [target]
        if _splash_target_objects:
            _carried_debuff_target_list.extend(_splash_target_objects)
        for sb in carried_debuff_buffs:
            ...
            # duration_owner: "caster"の場合、施法者行動終了時にduration減少(DURABLE_SOURCE_MANEUVER_END)
            # 130105 PS1 Lv11+「自身が次の行動を終えるまでの間」用
            debuff_duration_owner = hlf.get('carried_debuff_duration_owner', 'target')
            ...
            # timing_type: duration_owner="caster" -> DURABLE_SOURCE_MANEUVER_END, それ以外はDURABLE_TARGET_MANEUVER_END
            if debuff_duration_owner == 'caster':
                _carried_timing = AuraUpdateTiming.DURABLE_SOURCE_MANEUVER_END.value
            else:
                _carried_timing = AuraUpdateTiming.DURABLE_TARGET_MANEUVER_END.value
            # carried_debuffを全対象(主目標+splash目標)に付与
            for _cd_target in _carried_debuff_target_list:
                # carried_debuff uses own alive check (not skip_damage_buffs), so it can apply to OVERKILL/fully-evaded targets
                if not _cd_target.is_alive:
                    continue
                # 在add_aura前后各算一次属性，构造"ATK:前值→后值"格式detail（追踪真实属性，非效果量加减法）
                stat_before = self._compute_stat_display(_cd_target, mapped_type)
                carried_aura = BuffState(
                    buff_id=f"{caster.unit_id}_{mapped_type}_{_cd_target.unit_id}_enchant",
                    name=mapped_type,
                    effect_type=mapped_type,
                    value=debuff_value,
                    duration=debuff_duration,
                    timing_type=_carried_timing,
                ...
                added = self.aura_service.add_aura(_cd_target, carried_aura)
                if not added:
                    _log.info("[ENCHANT_DMG] %s: carried_debuff %s BLOCKED on %s (immunity/buff_block), skipping",
                              caster.name, debuff_type, _cd_target.name)
                    continue
                _log.info("[ENCHANT_DMG] %s: carried_debuff %s(%.4f, tag=%d) applied to %s (dur=%d, stackable=%s, owner=%s)",
                          caster.name, debuff_type, debuff_value, debuff_value_tag, _cd_target.name, debuff_duration, debuff_stackable, debuff_duration_owner)
                stat_after = self._compute_stat_display(_cd_target, mapped_type)
                ...
                self._per_target_enchant_cache["carried_debuff_targets"].append({
                    "target_id": _cd_target.unit_id,
                    "target": _cd_target.unit_id,
                    "effect": mapped_type,
        ...
        self._per_target_enchant_cache["damage_delta"] += total_damage_delta
        return total_damage_delta

    def _finalize_enchant_buffs_after_damage_effect(self, caster: UnitState,
                                                     add_damage_buffs: list,
                                                     enchant_buffs: list,
                                                     carried_debuff_buffs: list) -> None:
        """Decrement hit_limited for enchant-related buffs after a damage effect's per-target loop ends.

        Called once per damage effect (after all targets processed) to ensure hit_limited
        is consumed once per attack, not once per target.
        sub_unit_buffs have no hit_limited consumption (persistent until sub_unit_hp reaches 0).
        """
        for ab in add_damage_buffs:
            # hit_limited=0 means unlimited (persistent add_damage buff)
            if ab.hit_limited > 0:
                ab.hit_limited -= 1
                _log.info("[ADD_DMG] %s: add_damage buff hit_limited %d->%d",
                          caster.name, ab.hit_limited + 1, ab.hit_limited)
                if ab.hit_limited <= 0:
                    caster.buffs = [b for b in caster.buffs if b.buff_id != ab.buff_id]
                    _log.info("[ADD_DMG] %s: add_damage buff EXPIRED", caster.name)

        for eb in enchant_buffs:
            # hit_limited=0 means unlimited (persistent enchant buff, e.g. 110069 永続 add_damage_to_attack)
            if eb.hit_limited > 0:
                eb.hit_limited -= 1
    ...
    def _apply_block_enchant_damage(self, caster: UnitState, targets_hit: list,
        ...
        Reads from self._per_target_enchant_cache (populated by _process_enchant_damage_for_target
        during the per-target loop in _apply_damage/_apply_hp_ratio_damage/_apply_damage_special).
        """
        ...
        enchant_targets = self._per_target_enchant_cache["enchant_targets"]
        splash_targets = self._per_target_enchant_cache.get("splash_targets", [])
        carried_debuff_targets = self._per_target_enchant_cache["carried_debuff_targets"]
        damage_delta = self._per_target_enchant_cache["damage_delta"]

        # 附魔伤害+sub_unit伤害累加到技能累计伤害，供延迟执行的lifesteal使用
        if damage_delta > 0:
            self._most_recent_damage += damage_delta
            _log.info("[ENCHANT_BLOCK] %s: enchant+sub_unit damage %d accumulated to _most_recent_damage (total=%d)",
                      caster.name, damage_delta, self._most_recent_damage)

        results = []
        # splash_targetsとenchant_targetsを統合して返す（どちらもダメージ効果）
        all_damage_targets = enchant_targets + splash_targets
        if all_damage_targets:
            _log.info("[ENCHANT_BLOCK] %s: enchant_targets=%s splash_targets=%s, new_total=%d", caster.name,
    ...
    def _apply_on_crit_blocks(self, caster: UnitState, target: UnitState,
        ...
        for block in self._on_crit_blocks:
            ...
            # 支持直接type=on_crit或and条件中包含on_crit子条件
            block_condition = getattr(block, 'condition', None)
            once_per_skill = True
            sub_conditions = []
            if isinstance(block_condition, dict):
                if block_condition.get('type') == 'on_crit':
                    ...
                    sub_conditions = block_condition.get('conditions', [])
                    on_crit_sub = next((sc for sc in sub_conditions
                                         if isinstance(sc, dict) and sc.get('type') == 'on_crit'), None)
                    if on_crit_sub:
                        once_per_skill = on_crit_sub.get('once_per_skill', True)
            ...
            # 评估and条件中的其他子条件（active_level_min/mark_count等，on_crit已由暴击触发满足）
            skip_block = False
            for sub_cond in sub_conditions:
                ...
                st = sub_cond.get('type')
                if st == 'on_crit':
                    continue  # on_crit本身已由暴击触发满足
                if st == 'active_level_min':
                    _min_level = sub_cond.get('value', 0)
                    if skill_level < _min_level:
                        _log.info("[ON_CRIT] %s: skipping block %d (active_level_min: %d < %d)",
                                  caster.name, block.block_id, skill_level, _min_level)
                        skip_block = True
                        break
                elif st == 'mark_count':
                    # 检查暴击目标的mark数量（如120077 Block2: 暴击目标已有3+「ターゲット」mark）
                    mark_name = sub_cond.get('mark_name', '')
                    op = sub_cond.get('operator', '>=')
                    val = sub_cond.get('value', 0)
                    target_scope = sub_cond.get('target_scope', 'attacked')
                    # on_crit场景下attacked scope使用当前暴击目标
                    check_targets = []
                    if target_scope == 'enemy':
                        from ...entities_v2.enums import Side as _SideOC
                        opposing_team = (battlefield.friend_team if caster.side == _SideOC.ENEMY
                                         else battlefield.enemy_team)
                        for u in opposing_team:
                            if u.is_alive:
                                check_targets.append(u)
                        ...
                        # 默认检查当前暴击目标
                        check_targets = [target] if target and target.is_alive else []
                    matched = False
                    for check_unit in check_targets:
                        mc = sum(1 for d in check_unit.debuffs
                                  if d.effect_type == SkillEffectType.MARK.value and getattr(d, 'name', '') == mark_name)
                        mc += sum(1 for b in check_unit.buffs
                                   if b.effect_type == SkillEffectType.MARK.value and getattr(b, 'name', '') == mark_name)
                        # _eval_block_condition为模块级函数（line 91）
                        if _eval_block_condition(mc, op, val):
                            matched = True
                            _log.info("[ON_CRIT] %s: mark_count '%s' on %s = %d %s %d -> PASS",
                                      caster.name, mark_name, check_unit.name, mc, op, val)
                            break
                        else:
                            _log.info("[ON_CRIT] %s: mark_count '%s' on %s = %d %s %d -> FAIL",
                                      caster.name, mark_name, check_unit.name, mc, op, val)
                    if not matched:
                        _log.info("[ON_CRIT] %s: skipping block %d (mark_count '%s' %s %d not met on crit target)",
                                  caster.name, block.block_id, mark_name, op, val)
                        skip_block = True
                        break
            if skip_block:
                continue
                ...
                if effect_type == "damage":
                    ...
                    for dt in dmg_targets:
                        ...
                        if hp_loss > 0:
                            ...
                            # 130158 ヒートアップ・ラブ: 累计当次行动中受到的伤害
                            dt._damage_taken_in_action = getattr(dt, '_damage_taken_in_action', 0) + hp_loss
                    ...
                    # is_buff_mark=false (debuff mark) → is_debuff=True
                    # is_buff_mark=true (buff mark) → is_debuff=False
                    _mark_flags = getattr(effect, 'flags', {}) or {}
                    _mark_is_debuff = not _mark_flags.get('is_buff_mark', False)
                    aura_result = self._apply_aura(caster, effect, battlefield, is_debuff=_mark_is_debuff)
                    if aura_result:
                        self._on_crit_effects.append(aura_result)
    ...
    def _apply_heal(self, caster: UnitState, effect, battlefield: BattlefieldState) -> Optional[Dict]:
        ...
        # value_tag 解析：支持基于技能等级的动态百分比（如130158 cure1/cure2）
        if not heal_pct and getattr(effect, 'value_tag', None):
            resolved_pct = self._resolve_tag_value_for_caster(caster, effect, effect.value_tag)
            if resolved_pct is not None:
                heal_pct = resolved_pct
                _log.info("[HEAL] %s: resolved value_tag '%s' -> %.1f (Lv%d)",
                          caster.name, effect.value_tag, heal_pct,
                          caster.skill_levels.get(self._current_skill_id, 1))
        ...
        # value_source (top-level): 130158 ヒートアップ・ラブ 使用 damage_taken_in_action 作为治疗基数
        _heal_value_source = getattr(effect, 'value_source', None)
        if _heal_value_source == "damage_taken_in_action":
            heal_base = "damage_taken_in_action"
        ...
        # split_among_targets: 总回復量（ATK×威力%）を味方人数で均等配分
        # 例: AS3 120102 マーシーズ・エンブレイス「威力分のHP回復量を均等に配分して回復」
        # 注意: 均等配分はATK基数のみ対応（max_hp/lost_hp基数は対象別計算のため適用外）
        split_among_targets = heal_flags.get('split_among_targets', False)
        alive_targets_count = sum(1 for t in targets if t.is_alive)
        if split_among_targets and alive_targets_count > 0 and heal_base == 'atk':
            total_heal_pool = int(effective_atk * heal_pct / 100)
            per_target_base = total_heal_pool // alive_targets_count
            _log.info("[HEAL] %s: split_among_targets total_pool=%d / %d alive = per_target_base=%d",
                      caster.name, total_heal_pool, alive_targets_count, per_target_base)
        else:
            per_target_base = None  # None = 従来通りのper-target計算
        ...
        for target in targets:
            ...
            if per_target_base is not None:
                # split_among_targets: 均等配分された基数を使用
                heal_amount = per_target_base
            elif heal_base == 'max_hp':
            ...
            elif heal_base == 'consume_hp':
                # 基于本技能consume_hp消费的HP量（如120154 私に任せて！）
                consumed = getattr(self, '_hp_consumed', 0)
                heal_amount = int(consumed * heal_pct / 100)
                _log.info("[HEAL] %s -> %s: heal_base=consume_hp consumed=%d heal_pct=%d%%",
                          caster.name, target.name, consumed, heal_pct)
            elif heal_base == 'damage_taken_in_action':
                # 130158 ヒートアップ・ラブ: 基于目标当次行动中受到的伤害计算治疗量
                # _damage_taken_in_action 由 battle_flow_controller 在伤害结算时累加
                dmg_taken = getattr(target, '_damage_taken_in_action', 0)
                heal_amount = int(dmg_taken * heal_pct / 100)
                _log.info("[HEAL] %s -> %s: heal_base=damage_taken_in_action dmg_taken=%d heal_pct=%.1f%%",
                          caster.name, target.name, dmg_taken, heal_pct)
            else:
    ...
    def _apply_aura(self, caster: UnitState, effect, battlefield: BattlefieldState, is_debuff: bool) -> Optional[Dict]:
        ...
        # P0-2 阶段1：目标选择/过滤逻辑独立为 _select_aura_targets 方法
        # 返回值可能为 None（crit_target/triggered_by 无目标时应立即返回None）
        _aura_targets_result = self._select_aura_targets(
            caster, effect, battlefield, is_debuff
        )
        if _aura_targets_result is None:
            return None
        targets, effect_flags_aura, carried_debuff = _aura_targets_result

        # P0-2 阶段2：value/duration/timing 计算逻辑独立为 _compute_aura_value_duration 方法
        # effect_flags_aura 可能被方法内修改（如设置 attack_limited/skip_restore）
        value, duration, hit_limited_from_dur, timing, original_dur_type = self._compute_aura_value_duration(
            caster, effect, battlefield, effect_flags_aura
        )
        # dur_type 用于后续 aura_detail_dict 显示，与 _compute_aura_value_duration 内部保持一致
        dur_type = original_dur_type or "action"
        ...
        for target in targets:
            ...
            # P0-2 阶段3：per-target 跳过检查独立为 _check_aura_target_skip 方法
            # 副作用：消费 debuff_immune/evade buff，记录 _debuff_immune_blocked_targets/_skill_evaded_targets
            skip, actual_is_debuff, blocked_detail = self._check_aura_target_skip(
                caster, target, effect, effect_flags_aura, is_debuff, carried_debuff,
                mapped_effect_type, effect_condition, _skip_per_target_condition
            )
            if blocked_detail:
                blocked_details.append(blocked_detail)
            if skip:
                continue

            # P0-2 阶段5-1：shield 特殊处理独立为 _apply_aura_shield_effect 方法
            # 修改 target.shield/physical_shield/en_shield 字段，返回实际盾值
            _shield_value_for_buff = self._apply_aura_shield_effect(
                caster, target, effect, effect_flags_aura, value
            )

            # P0-2 阶段4：BuffState构造和属性设置独立为 _build_aura_buff_state 方法
            # 注意：shield特殊处理已在上方完成，_shield_value_for_buff 传入用于BuffState.shield_amount
            aura, final_value, resolved_value_tag = self._build_aura_buff_state(
                caster, target, effect, effect_flags_aura, battlefield,
                mapped_effect_type, actual_is_debuff, carried_debuff,
                value, duration, hit_limited_from_dur, timing, original_dur_type,
                _shield_value_for_buff
            )
            _ = _bidir_link_map  # 占位变量保留（_build_aura_buff_state内部已处理bidir_link）

            # P0-2 阶段5-2：Knockout refresh 处理独立为 _handle_knockout_refresh 方法
            # 检查目标是否已有Knockout debuff，保留duration更长的
            is_new_knockout, _should_skip_knockout = self._handle_knockout_refresh(
                caster, target, mapped_effect_type, value, duration, dur_type, aura_details
            )
            if _should_skip_knockout:
                continue

            # P0-2 阶段5-3：max_hp_up 处理独立为 _apply_max_hp_up 方法
            # 直接修改unit.max_hp（基于原始base max_hp计算增量），不增加current_hp，不添加buff
            if effect.effect_type == "max_hp_up":
                self._apply_max_hp_up(
                    caster, target, mapped_effect_type, value, duration, dur_type,
                    resolved_value_tag, aura_details
                )
                continue

            # P0-2 阶段5-4：add_max_ap 处理独立为 _apply_add_max_ap 方法
            # 直接修改unit.initial_active_point（提升AP上限），不增加current_ap，不添加buff
            if effect.effect_type == "add_max_ap":
                self._apply_add_max_ap(
                    caster, target, mapped_effect_type, value, duration, dur_type, aura_details
                )
                continue
            ...
            # 130160 サマータイム・ロマンス: mark value>1 一次性多层赋予
            # 当 effect.effect_type == "mark" 且 value > 1 且 stackable=true 时，
            # 复制 N 个独立 aura 并分别 add_aura（N = effect.value）
            # 每个 aura 需要独立的 buff_id（stackable=true 已确保 uuid 唯一化）
            _mark_multi_layer = (
                effect.effect_type == "mark"
                and isinstance(effect.value, (int, float))
                and effect.value > 1
                and bool(effect_flags_aura.get('stackable'))
            )
            if _mark_multi_layer:
                import copy as _copy_mod
                import uuid as _uuid_mod
                _mark_layers = int(effect.value)
                _added_count = 0
                for _layer_idx in range(_mark_layers):
                    if _layer_idx == 0:
                        _layer_aura = aura  # 第一层使用已构造的 aura
                    else:
                        _layer_aura = _copy_mod.copy(aura)
                        _layer_aura.buff_id = (
                            f"{caster.unit_id}_{mapped_effect_type}_{target.unit_id}_{_uuid_mod.uuid4().hex[:8]}"
                        )
                    _layer_added = self.aura_service.add_aura(target, _layer_aura)
                    if _layer_added:
                        _added_count += 1
                        if actual_is_debuff:
                            self._debuffs_applied_this_skill.add(_layer_aura.buff_id)
                _log.info("[AURA_APPLY] %s -> %s: mark multi-layer applied %d/%d layers (mark_name='%s')",
                          caster.name, target.name, _added_count, _mark_layers,
                          effect_flags_aura.get('mark_name', ''))
                aura_added = _added_count > 0
            else:
                aura_added = self.aura_service.add_aura(target, aura)
            if not aura_added:
                _log.info("[AURA_APPLY] %s -> %s: SKIPPED (blocked by immunity/BlockSpecificAura)",
                          caster.name, target.name)
                continue
            if actual_is_debuff and not _mark_multi_layer:
            ...
            if carried_debuff:
                ...
                aura_detail = f"护盾+{_shield_value_for_buff}"
            elif stat_before[0]:
    ...
    def _apply_aura_shield_effect(
        self,
        caster: UnitState,
        target: UnitState,
        effect,
        effect_flags_aura: dict,
        value: int,
    ) -> int:
        """处理shield效果的盾值计算和分配。

        涵盖原 _apply_aura L5747-L5781 的 shield 特殊处理逻辑：
        - 根据 shield_base (atk/max_hp/consume_hp) 计算盾值
        - 根据 damage_element 分配到 shield/physical_shield/en_shield 字段
        - 修改 target 的盾字段值
        ...
        Args:
            ...
            target: 当前目标单位
            effect: 技能效果对象
            effect_flags_aura: effect.flags 字典
            value: 基础value（含hp_scaling）

        Returns:
            shield_value: 实际盾值（用于BuffState.shield_amount），
                          非shield效果返回0
        """
        ...
        if shield_base == 'max_hp':
            ...
            shield_base_value = effective_max_hp
        elif shield_base == 'consume_hp':
            # 基于本技能consume_hp消费的HP量（如AS1 120121 風紀を守る一撃 Lv11+）
            consumed = getattr(self, '_hp_consumed', 0)
            shield_value = int(consumed * value / 100)
            shield_base_value = consumed
        else:
            effective_atk = self.damage_service._calculate_final_stat(caster, "attack")
        ...
        if shield_elem == 'physical':
            ...
            _log.info("[AURA_APPLY] %s -> %s: +physical_shield %d (total=%d, base=%s %.0f%% × %.1f%%)",
                      ...
                      shield_base, shield_base_value, value)
        elif shield_elem == 'en':
    ...
    def _handle_knockout_refresh(
        self,
        caster: UnitState,
        target: UnitState,
        mapped_effect_type: str,
        value: int,
        duration: int,
        dur_type: str,
        aura_details: list,
    ) -> Tuple[bool, bool]:
        """处理Knockout refresh规则。

        涵盖原 _apply_aura L5763-L5792 的 Knockout refresh 逻辑：
        - 检查目标是否已有Knockout debuff
        - 如果有，保留duration更长的，跳过添加重复Knockout
        - 标记是否为新的knockout（用于PS2 trigger filtering）

        Args:
            caster: 施法者
            target: 当前目标单位
            mapped_effect_type: 映射后的effect_type字符串
            value: 基础value
            duration: 持续时间
            dur_type: duration_type字符串
            aura_details: aura详情列表（可能追加knockout_refresh记录）

        Returns:
            (is_new_knockout, should_skip) 二元组：
            - is_new_knockout: 是否为新的knockout（True=新施加，False=已有knockout被保留）
            - should_skip: 是否应跳过当前目标（True=已有knockout且作为refresh记录处理，跳过add_aura）
        """
        ...
        if mapped_effect_type == SkillEffectType.KNOCKOUT.value:
            ...
            if existing_stun:
                ...
                return is_new_knockout, True  # should_skip=True
        return is_new_knockout, False

    def _apply_max_hp_up(
        self,
        caster: UnitState,
        target: UnitState,
        mapped_effect_type: str,
        ...
        duration: int,
        dur_type: str,
        resolved_value_tag: int,
        aura_details: list,
    ) -> None:
        """处理max_hp_up效果。

        涵盖原 _apply_aura L5770-L5795 的 max_hp_up 逻辑：
        ...
        Args:
            ...
            duration: 持续时间
            dur_type: duration_type字符串
            resolved_value_tag: 0=百分比, 1=固定值
            aura_details: aura详情列表（追加max_hp_up记录）
        """
    ...
    def _apply_add_max_ap(
        ...
        涵盖原 _apply_aura L5797-L5818 的 add_max_ap 逻辑：
        - 直接修改target.initial_active_point（提升AP上限）
        - 不增加current_ap，不添加buff
        - 配合后续add_ap effect提升当前AP资源（如130075 イモータル・ヴァンパイア）
        ...
        Args:
            ...
            duration: 持续时间
            dur_type: duration_type字符串
            aura_details: aura详情列表（追加add_max_ap记录）
        """
        ap_increase = int(value)
        old_max_ap = target.initial_active_point
        target.initial_active_point += ap_increase
        _log.info("[ADD_MAX_AP] %s -> %s: max_ap %d -> %d (+%d), current_ap %d (unchanged)",
                  caster.name, target.name, old_max_ap, target.initial_active_point,
                  ap_increase, target.current_ap)
        aura_detail_dict = {
            "target": target.name,
            "target_id": target.unit_id,
            "effect": mapped_effect_type,
            ...
            "source": caster.name,
            "source_id": caster.unit_id,
            "detail": f"最大AP+{ap_increase}({old_max_ap}->{target.initial_active_point})",
        }
        aura_details.append(aura_detail_dict)

    def _select_aura_targets(
        self,
        caster: UnitState,
        effect,
        battlefield: BattlefieldState,
        is_debuff: bool,
    ) -> Optional[Tuple[List[UnitState], dict, bool]]:
        """选择aura效果的目标列表并应用各种过滤/重定向规则。

        涵盖原 _apply_aura 的目标选择/过滤/特殊重定向逻辑：
        - target_count扩展与_AURA_SPECIAL_POSTFILTER_TYPES扩展
        - cached_targets / trigger_attacker / triggered_by / attacked_targets / debuff_applied_target 等多种目标解析路径
        - trigger_attacker 覆盖（敌方/友方）
        - element / char_type 过滤
        - 特殊 target_type 处理（ally_front_row/enemy_front_row/attacker_row/enemy_row_others/ally_front）
        - 特殊 postfilter（highest_atk/lowest_atk/highest_spd/hp_ratio系列/mark_count/furthest）
        - lowest_hp_priority / en_type_priority / lowest_hp_row_only / front_priority / nearest_ally / behind_self / healed_target
        - target_count限制
        - cover替换
        ...
        Returns:
            (targets, effect_flags_aura, carried_debuff) 三元组：
            - targets: 最终目标单位列表
            - effect_flags_aura: effect.flags 字典（供 _apply_aura 后续逻辑使用）
            - carried_debuff: 是否为carried_debuff载荷（供 _apply_aura per-target 循环使用）

            或 None（当 crit_target/triggered_by 无可用目标时，应立即终止 _apply_aura）

        从 _apply_aura L5659-L6305 提取，保持原逻辑等价。
        """
        ...
        cached_targets = getattr(self, '_block_damage_targets', None)
        _eff_tid = getattr(effect, 'target_identifier', None)
        if effect.target_type == "crit_target":
            ...
            if crit_target and crit_target.is_alive:
                ...
                return None
        elif effect.target_type == "drawn_target":
            # drawn_target: 来自 random_draw 结果的目标 (110072 ミッドサマー・ラブ)
            # _current_drawn_targets 由前置 block 的 random_draw effect 设置
            # 在 _evaluate_block_condition 中已根据 drawn_target_is_enemy / drawn_target_is_ally 过滤
            drawn = getattr(self, '_current_drawn_targets', None)
            if not drawn:
                _log.info("[AURA_APPLY] %s: drawn_target unavailable, no _current_drawn_targets", caster.name)
                return None
            targets = [u for u, _t in drawn if u.is_alive]
            _log.info("[AURA_APPLY] %s: drawn_target -> %s", caster.name, [t.name for t in targets])
        elif _eff_tid == "trigger_attacker":
            # trigger_attacker: 反撃系PS/on_ally_killed系PS对触发源施加aura
            # (如130086 キャッチライト: on_ally_killed触发对killer施加mark和dmg_taken_up)
            ta = getattr(self, '_trigger_attacker', None)
            if ta and ta.is_alive:
                targets = [ta]
                _log.info("[AURA_APPLY] %s: trigger_attacker -> %s", caster.name, ta.name)
            else:
                # 无trigger_attacker上下文(直接execute_skill测试等): fall through到select_targets
                _log.info("[AURA_APPLY] %s: trigger_attacker not available, fall through to select_targets",
                          caster.name)
                targets = self.target_service.select_targets(
                    aura_skill_obj, caster, battlefield
                )
        elif _eff_tid == "triggered_by":
            primary = getattr(self, '_primary_target', None)
            if primary and primary.is_alive:
                targets = [primary]
            ...
            for u in skill_attacked:
                if u.unit_id not in seen and u.is_alive:
                    seen.add(u.unit_id)
                    all_attacked.append(u)
            # 回退: 触发器上下文的_damaged_targets（如PS2 after_as_attack触发时，
            # _skill_all_attacked_targets已被execute_skill重置，需从trigger context获取）
            damaged_from_trigger = getattr(self, '_damaged_targets', None) or []
            for u in damaged_from_trigger:
                if u.unit_id not in seen and u.is_alive:
                    seen.add(u.unit_id)
                    all_attacked.append(u)
            # 回退: _last_primary_target（如after_as_attack的primary_target）
            if not all_attacked:
                primary = getattr(self, '_last_primary_target', None) or getattr(self, '_primary_target', None)
                if primary and primary.is_alive and primary.unit_id not in seen:
                    all_attacked.append(primary)
                    seen.add(primary.unit_id)
        ...
        if effect.target_type == "enemy_front_row":
            # enemy_front_row: 所有前排敌方（如 ルミナス・ガード 130110 Lv11+ 敵前衛デバフ）
            from src.entities_v2.enums import Side as _SideEFR
            enemy_team = battlefield.enemy_team if caster.side == _SideEFR.ALLY else battlefield.friend_team
            front_row_enemies = [u for u in enemy_team if u.is_alive and u.position.value.endswith('_front')]
            if front_row_enemies:
                targets = front_row_enemies
                _log.info("[AURA_APPLY] %s: enemy_front_row -> %s",
                          caster.name, [t.name for t in targets])
            else:
                _log.info("[AURA_APPLY] %s: enemy_front_row -> no front-row enemies", caster.name)
        ...
        if effect.target_type == "enemy_row_others":
            # enemy_row_others: 前block主目标所在横一列中的其他敌方（排除主目标自身）
            # 参考 スタンサプレッション(120054) Block2 atk_down「対象と同じ横一列にいる他の敵」
            # 主目标来源优先级：_last_primary_target > _prev_block_damage_targets
            primary_target = getattr(self, '_last_primary_target', None)
            if primary_target is None or not primary_target.is_alive:
                prev_targets = getattr(self, '_prev_block_damage_targets', {})
                for _pk, _pv in prev_targets.items():
            ...
            if primary_target is None or not primary_target.is_alive:
                targets = []
                _log.info("[AURA_APPLY] %s: enemy_row_others -> no primary_target available, skip",
                          caster.name)
            else:
                from src.entities_v2.enums import Side as _SideERO
                enemy_team = battlefield.enemy_team if caster.side == _SideERO.ALLY else battlefield.friend_team
                anchor_is_front = self.target_service._is_front_row(primary_target)
                row_others = [u for u in enemy_team if u.is_alive
                              and self.target_service._is_front_row(u) == anchor_is_front
                              and u.unit_id != primary_target.unit_id]
                targets = row_others
                _log.info("[AURA_APPLY] %s: enemy_row_others -> %s (primary=%s, %s row)",
                          caster.name, [t.name for t in targets], primary_target.name,
                          "FRONT" if anchor_is_front else "BACK")
        ...
        if effect.target_type and "highest_atk" in effect.target_type:
            cache = getattr(self, '_block_aura_target_cache', None)
            if cache is not None and effect.target_type in cache:
                targets = list(cache[effect.target_type])
                if targets:
                    best_atk = self.damage_service._calculate_final_stat(targets[0], "attack") if self.damage_service else targets[0].attack
                    _log.info("[AURA_APPLY] %s: highest_atk filter -> %s (atk=%d) [cached]",
                              caster.name, targets[0].name, best_atk)
            elif targets:
                best = self.target_service.select_max_with_stealth(
                    targets,
                    key_func=lambda u: self.damage_service._calculate_final_stat(u, "attack") if self.damage_service else u.attack,
                ...
                if cache is not None and best:
                    cache[effect.target_type] = list(targets)
                best_atk = self.damage_service._calculate_final_stat(best, "attack") if self.damage_service else best.attack
                _log.info("[AURA_APPLY] %s: highest_atk filter -> %s (atk=%d)",
                          caster.name, best.name, best_atk)

        if effect.target_type and "lowest_atk" in effect.target_type:
            cache = getattr(self, '_block_aura_target_cache', None)
            if cache is not None and effect.target_type in cache:
                targets = list(cache[effect.target_type])
                if targets:
                    best_atk = self.damage_service._calculate_final_stat(targets[0], "attack") if self.damage_service else targets[0].attack
                    _log.info("[AURA_APPLY] %s: lowest_atk filter -> %s (atk=%d) [cached]",
                              caster.name, targets[0].name, best_atk)
            elif targets:
                best = self.target_service.select_min_with_stealth(
                    targets,
                ...
                if cache is not None and best:
                    cache[effect.target_type] = list(targets)
                best_atk = self.damage_service._calculate_final_stat(best, "attack") if self.damage_service else best.attack
                _log.info("[AURA_APPLY] %s: lowest_atk filter -> %s (atk=%d)",
                          caster.name, best.name, best_atk)
        ...
        if effect.target_type and "highest_spd" in effect.target_type:
            ...
            if cache is not None and effect.target_type in cache:
                targets = list(cache[effect.target_type])
                if targets:
                    best_spd = self.damage_service._calculate_final_stat(targets[0], "speed") if self.damage_service else targets[0].speed
                    _log.info("[AURA_APPLY] %s: highest_spd filter -> %s (spd=%d) [cached]",
                              caster.name, targets[0].name, best_spd)
            elif targets:
                best = self.target_service.select_max_with_stealth(
                    targets,
                    key_func=lambda u: self.damage_service._calculate_final_stat(u, "speed") if self.damage_service else u.speed,
        ...
        if effect.target_type == "enemy_single_lowest_max_hp":
            if targets:
                best = self.target_service.select_min_with_stealth(
                    targets,
                    key_func=lambda u: (u.max_hp, self.target_service._get_sort_key(caster, u)),
        ...
        if effect.target_type and "furthest" in effect.target_type:
            ...
            if targets:
                ...
                targets = [best] if best else []
                if best:
                    _log.info("[AURA_APPLY] %s: furthest filter -> %s",
                              caster.name, best.name)
        ...
        return targets, effect_flags_aura, carried_debuff

    def _compute_aura_value_duration(
        self,
        caster: UnitState,
        effect,
        battlefield: BattlefieldState,
        ...
        """计算aura效果的value、duration、timing等参数。

        涵盖原 _apply_aura L5668-L5757 的 value/duration/timing 计算逻辑：
        - hp_scaling: 基于caster或最近敌方HP比例缩放value
        - duration_type: 决定duration和timing（action/hit/attack/attacker_action/skill/turn/其他）
        - hit_limited_from_dur: 从duration提取的hit_limited值（仅duration_type=hit时）
        - 特殊处理：add_damage_to_attack with hit_limited>0 时设置 skip_restore=True, duration=1
        - 特殊处理：duration_type=attack 且非 hit_limited_lifecycle 时设置 attack_limited=1

        注意：effect_flags_aura 是可变 dict，方法内部可能修改其内容
       （如设置 attack_limited/skip_restore），修改会反映到调用方。

        Args:
            caster: 施法者
            effect: 技能效果对象
            battlefield: 战场状态
            effect_flags_aura: effect.flags 字典（可能被修改）

        Returns:
            (value, duration, hit_limited_from_dur, timing, original_dur_type) 五元组：
            - value: 计算后的值（含hp_scaling）
            - duration: 持续时间（-1表示永续，由hit_limited/attack_limited控制）
            - hit_limited_from_dur: 从duration提取的hit_limited值
            - timing: AuraUpdateTiming值
            - original_dur_type: 原始duration_type字符串（用于清理）
        """
        # === value 计算（含 hp_scaling + value_tag 解析） ===
        # value_tag 解析：支持基于技能等级的动态值（如 PS3 130163 evade 的 kaihi%、dmg_dealt_up 的 dmg%）
        # 当 effect.value 为 None 且配置了 value_tag 时，从技能等级数据解析对应 tag 的数值
        if effect.value is None and getattr(effect, 'value_tag', None):
            resolved = self._resolve_tag_value_for_caster(caster, effect, effect.value_tag)
            value = float(resolved) if resolved is not None else 0
            if resolved is not None:
                _log.info("[AURA_APPLY] %s: resolved value_tag '%s' -> %.1f (Lv%d)",
                          caster.name, effect.value_tag, value,
                          caster.skill_levels.get(self._current_skill_id, 1))
        else:
        ...
        # === duration / hit_limited_from_dur 计算 ===
        ...
        return value, duration, hit_limited_from_dur, timing, original_dur_type

    def _check_aura_target_skip(
        self,
        caster: UnitState,
        target: UnitState,
        effect,
        ...
        mapped_effect_type: str,
        effect_condition,
        skip_per_target_condition: bool,
    ) -> Tuple[bool, bool, Optional[dict]]:
        """检查aura效果对当前目标是否应跳过。

        涵盖原 _apply_aura L5732-L5894 的 per-target 跳过检查逻辑：
        - _check_target_condition 条件检查（除非 skip_per_target_condition）
        - carried_debuff 转换（同阵营时 actual_is_debuff=False）
        - mark with is_buff_mark 转换（actual_is_debuff=False）
        - skip_if_exists 检查（mark 已有则跳过）
        - max_stacks 检查（mark 达上限则跳过）
        - debuff_immune 检查（消费 immune，记录 blocked）
        - linked_mark 检查（mark 被免疫/闪避则跳过绑定 debuff）
        - _skill_evaded_targets 检查（含清廉なる一撃特殊处理）
        - evade buff 检查（含 sure_hit 优先，消费 evade）

        副作用（方法内执行）：
        - 消费 debuff_immune（_consume_debuff_immune）
        - 消费 evade buff（hit_limited -= 1，可能移除）
        - 添加到 _debuff_immune_blocked_targets
        - 添加到 _skill_evaded_targets
        ...
        Args:
            ...
            effect: 技能效果对象
            effect_flags_aura: effect.flags 字典
            is_debuff: 是否为debuff（原始值）
            carried_debuff: 是否为carried_debuff载荷
            mapped_effect_type: 映射后的effect_type字符串
            effect_condition: effect.condition 字典（可能为None）
            skip_per_target_condition: 是否跳过per-target条件检查

        Returns:
            (skip, actual_is_debuff, blocked_detail) 三元组：
            - skip: True表示跳过此目标
            - actual_is_debuff: 修正后的is_debuff（可能因carried_debuff/is_buff_mark转为False）
            - blocked_detail: 被免疫/闪避时的详情字典（调用方追加到blocked_details），None表示无
        """
        # === per-target 条件检查 ===
        if not skip_per_target_condition and not self._check_target_condition(target, effect_condition):
        ...
        # === skip_if_exists: 若目标已有同名mark则跳过付与 ===
        if (effect.effect_type == "mark" and effect_flags_aura.get('skip_if_exists')):
            ...
            if already_has:
                ...
                return True, actual_is_debuff, None

        # === max_stacks: 若目标已持有同名mark达到上限则跳过 ===
        ...
        if actual_is_debuff and self._has_debuff_immune(target):
            ...
            return True, actual_is_debuff, blocked_detail
        ...
        if actual_is_debuff and caster.side != target.side:
            ...
            if evade_buffs:
                ...
                if sure_hit_buffs:
                    ...
                    # 技能级别闪避：同一技能内后续所有debuff效果也全部跳过（仅消耗1层闪避）
                    self._skill_evaded_targets.add(target.unit_id)
                    _log.info("[EVADE] %s: added to _skill_evaded_targets (rest of skill will skip)", target.name)
                    # 记录被闪避的目标，供linked_mark检查使用
                    self._debuff_immune_blocked_targets.add(target.unit_id)
                    blocked_detail = {
                        "target": target.name,
        ...
        # 所有检查通过，不跳过
        return False, actual_is_debuff, None

    def _build_aura_buff_state(
        self,
        caster: UnitState,
        target: UnitState,
        effect,
        ...
        battlefield: BattlefieldState,
        mapped_effect_type: str,
        actual_is_debuff: bool,
        carried_debuff: bool,
        value: int,
        duration: int,
        hit_limited_from_dur: int,
        timing: str,
        original_dur_type: str,
        shield_value_for_buff: int = 0,
    ) -> Tuple["BuffState", int, int]:
        """构造 BuffState 对象并设置所有属性。

        涵盖原 _apply_aura L5781-L6075 的逻辑（不含 shield 特殊处理 L5747-L5779）：
        - aura_name 设置（mark_name/buff_name）
        - resolved_value_tag 解析（0=percent, 1=fixed）
        - attack_limited_val 计算（含 guard 新旧版本区分）
        - _aura_source_unit_id 计算（双向 damage_link 占位，当前恒为 caster）
        - final_value 计算（scale_by_target_hp_ratio / scale_by_alive_allies /
          value_scales_with_self_mark_count / double_for_character_type）
        - _hlf 构造（carried_debuff payload / add_status / splash_to_adjacent）
        - BuffState 创建
        - BuffState 属性设置（damage_element / shield_amount / stackable /
          stack_count / is_memory_buff / threshold_pct / caster_alive /
          unremovable / skip_restore / hp_threshold / mark_condition /
          hp_ratio_dynamic / hp_ratio_dynamic_direct / hp_ratio_dynamic_inverse /
          target_hp_ratio_higher_than_self / attacker_hp_ratio_gt_self /
          linked_mark / linked_buff_id / original_duration_type /
          BlockSpecificAura / HOT heal_base 等）

        注意：shield 特殊处理（计算 shield_value 并修改 target.shield 等）由调用方
        在调用本方法之前执行，通过 shield_value_for_buff 参数传入实际盾值。
        ...
        Args:
            ...
            actual_is_debuff: 修正后的is_debuff（可能因carried_debuff/is_buff_mark转为False）
            carried_debuff: 是否为carried_debuff载荷
            value: 基础value（含hp_scaling，未含per-target缩放）
            duration: 持续时间
            hit_limited_from_dur: 从duration提取的hit_limited值
            timing: AuraUpdateTiming值
            original_dur_type: 原始duration_type字符串
            shield_value_for_buff: shield实际盾值（仅shield效果有值，默认0）

        Returns:
            (aura, final_value, resolved_value_tag) 三元组：
            - aura: 构造好的 BuffState 对象（未添加到target）
            - final_value: 最终value（含per-target缩放）
            - resolved_value_tag: value_tag (0=percent, 1=fixed)
        """
        # === aura_name 设置 ===
        ...
        # === _aura_source_unit_id 计算（双向 damage_link 占位） ===
        # 双向ダメージリンク: source_unit_id指向配对目标而非施法者
        # 当前 damage_link 已独立存储，is_bidir_damage_link 恒为 False
        _bidir_link_map = {}
        is_bidir_damage_link = False
        ...
        if effect_flags_aura and effect_flags_aura.get('double_for_character_type'):
            _dct = effect_flags_aura.get('double_for_character_type')
            if target.character_type == _dct:
                _pre_dct = final_value
                final_value = final_value * 2
                _log.info("[AURA_APPLY] %s -> %s: double_for_character_type=%d value %.1f -> %.1f",
                          caster.name, target.name, _dct, _pre_dct, final_value)

        # === _hlf 构造（carried_debuff payload） ===
        _hlf = {}
        if carried_debuff:
            _hlf = {
                ...
                # 130105 PS1: duration_owner="caster" で「自身が次の行動を終えるまで」を実現
                'carried_debuff_duration_owner': effect_flags_aura.get('duration_owner', 'target') if effect_flags_aura else 'target',
            }
            if effect.effect_type == 'poison':
        ...
        # 130105 PS1: splash_to_adjacent flag传递到hit_limited_flags
        # 供_process_enchant_damage_for_target中的splash処理セクション识别
        _splash_flag = effect_flags_aura.get('splash_to_adjacent') if effect_flags_aura else None
        if _splash_flag:
            _hlf['splash_to_adjacent'] = True
        ...
        if effect.effect_type == "shield":
            aura.shield_amount = shield_value_for_buff
            ...
            if decay_pct > 0 and shield_value_for_buff > 0:
        ...
        # dmg_taken_down_threshold: HP阈值减伤 buff (130160 サマータイム・ロマンス)
        # threshold_pct: 阈值百分比 (如20表示20% current_hp)
        # threshold_base: 阈值基数来源 ("current_hp" / "max_hp")
        # hit_count: 减伤可生效的 hit 次数 (1/2/3)，存入 hit_limited 字段
        if effect.effect_type == "dmg_taken_down_threshold":
            _thr_pct = effect_flags_aura.get('threshold_pct', 0) if effect_flags_aura else 0
            _thr_base = effect_flags_aura.get('threshold_base', 'current_hp') if effect_flags_aura else 'current_hp'
            aura.threshold_pct = float(_thr_pct)
            aura.threshold_base = _thr_base
            # hit_count 直接作为 hit_limited（每次被击中消耗1次）
            _dtd_hit_count = getattr(effect, 'hit_count', 0) or 0
            if _dtd_hit_count > 0:
                aura.hit_limited = int(_dtd_hit_count)
            _log.info("[DMG_TAKEN_DOWN_THRESHOLD] %s -> %s: threshold_pct=%.2f threshold_base=%s hit_limited=%d value=%.2f",
                      caster.name, target.name, aura.threshold_pct, aura.threshold_base, aura.hit_limited, final_value)
        ...
        # source_death_remove标记：付与者死亡时此buff/debuff自动消失
        # （如130161 PS1 def_down「防御デバフは付与者が倒れると解除される」）
        if effect_flags_aura.get('source_death_remove'):
            aura.source_death_remove = True
            _log.info("[AURA_APPLY] %s: source_death_remove flag set on %s (caster=%s)",
                      caster.name, mapped_effect_type, caster.name)
        ...
        if effect_flags_aura.get('hp_ratio_dynamic_inverse'):
            aura.hp_ratio_dynamic_inverse = True
            _log.info("[AURA_APPLY] %s: hp_ratio_dynamic_inverse on %s (dynamic boost inverse based on holder HP ratio)",
                      caster.name, mapped_effect_type)
        ...
        # attacker_hp_ratio_gt_self: 条件性减伤（如130103），
        # 仅当攻击者HP比例高于自身时减伤才生效
        if effect_flags_aura.get('attacker_hp_ratio_gt_self'):
            aura.attacker_hp_ratio_gt_self = True
            _log.info("[AURA_APPLY] %s: attacker_hp_ratio_gt_self on %s (only when attacker HP ratio > self)",
                      caster.name, mapped_effect_type)
        ...
        # linked_buff_id: 当指定effect_type的buff消失时，此buff也连动消失
        # （如PS2 130130: stealth消失時、HOT/dmg_taken_down连动消失）
        linked_buff = effect_flags_aura.get('linked_buff_id') if effect_flags_aura else None
        if linked_buff:
            aura.linked_buff_id = linked_buff
            _log.info("[AURA_APPLY] %s: linked_buff_id set to '%s' on %s", caster.name, linked_buff, mapped_effect_type)
    ...
    def _apply_add_status(self, caster: UnitState, effect, battlefield: BattlefieldState) -> Optional[Dict]:
        ...
        # 跨block引用：当当前block的_block_damage_targets为空时，回退到_prev_block_damage_targets
        # （如AS1 Block2的add_status应引用Block1 damage选中的多目标）
        cached_targets = getattr(self, '_block_damage_targets', None)
        prev_cached_targets = getattr(self, '_prev_block_damage_targets', {})
        if effect.target_type == "crit_target":
            # crit_target: on_crit block中使用，指向当前暴击目标（如120077 Block2 knockout）
            crit_target = getattr(self, '_on_crit_target', None)
            if crit_target and crit_target.is_alive:
                targets = [crit_target]
                _log.info("[ADD_STATUS] %s: crit_target -> %s",
                          caster.name, crit_target.name)
            else:
                _log.info("[ADD_STATUS] %s: crit_target unavailable, no targets", caster.name)
                return None
        elif cached_targets is not None and isinstance(cached_targets, dict) and effect.target_type in cached_targets:
            targets = [t for t in cached_targets[effect.target_type] if t.is_alive]
            _log.info("[ADD_STATUS] %s: using cached damage targets for %s -> %s",
                      caster.name, effect.target_type, [t.name for t in targets])
        elif prev_cached_targets and effect.target_type in prev_cached_targets:
            targets = [t for t in prev_cached_targets[effect.target_type] if t.is_alive]
            _log.info("[ADD_STATUS] %s: using prev_block cached damage targets for %s -> %s",
                      caster.name, effect.target_type, [t.name for t in targets])
        else:
        ...
        # lowest_hp_priority for add_status: select ally with lowest HP ratio
        # 例: EX 110045 メイドの本能 - 毒付与も perfect_evasion/atk_up と同じく最低HP味方を対象にする
        if add_status_flags.get('lowest_hp_priority'):
            from src.entities_v2.enums import Side as _SideAS
            ally_team = battlefield.friend_team if caster.side == _SideAS.ALLY else battlefield.enemy_team
            all_allies = [u for u in ally_team if u.is_alive]
            if all_allies:
                all_allies.sort(key=lambda u: (int(u.current_hp / max(u.max_hp, 1) * 10000) / 10000,
                                               self.target_service._get_sort_key(caster, u)))
                targets = [all_allies[0]]
                _log.info("[ADD_STATUS] %s: lowest_hp_priority -> %s (hp_pct=%.1f%%)",
                          caster.name, targets[0].name,
                          targets[0].current_hp / max(targets[0].max_hp, 1) * 100)
        ...
        if status_type == "burn":
            ...
            # poison百分比: 优先使用 effect.value (从 value_tag 解析, 如 30.0=30%)
            # 否则回退到 flags.poison_damage_pct (默认10)
            if value and value > 0:
                poison_damage_pct = value
            else:
                poison_damage_pct = add_status_flags.get('poison_damage_pct', 10)
            value = poison_damage_pct / 100.0
            _log.info("[ADD_STATUS] %s: poison pct=%s -> value=%.3f",
        ...
        for target in targets:
            ...
            aura_added = self.aura_service.add_aura(target, aura)
            if not aura_added:
                _log.info("[ADD_STATUS] %s -> %s: SKIPPED (blocked by immunity/BlockSpecificAura)",
                          caster.name, target.name)
                continue
            status_detail = {
                "target": target.name,
    ...
    def _apply_remove_all_debuffs(self, caster: UnitState, effect, battlefield: BattlefieldState) -> Optional[Dict]:
        """解除目标所有debuff，排除回忆卡debuff和不可解除debuff

        用于PS2 130130 風紀委員長としての矜持: ターン開始時自身の全debuff解除
        """
        if not self.target_service:
            _log.info("[REMOVE_ALL_DEBUFFS] %s: target_service unavailable", caster.name)
            return None
        ...
        for target in targets:
            if not target.is_alive:
                continue
            # 排除回忆卡debuff和不可解除debuff
            to_remove = [d for d in target.debuffs
                         if not d.is_memory_buff and not d.unremovable]
            removed_names = [d.name for d in to_remove]
            for d in to_remove:
                target.debuffs.remove(d)
            count = len(to_remove)
            ...
            _log.info("[REMOVE_ALL_DEBUFFS] %s: removed %d debuffs from %s (kept %d memory/unremovable)",
                      caster.name, count, target.name,
                      len([d for d in target.debuffs if d.is_memory_buff or d.unremovable]))

        return {
            "effect_type": "remove_all_debuffs",
            "target_count": len(targets),
            "total_removed": total_removed,
            "removed_details": removed_details,
        }
    ...
    def _apply_block_buff_by_type(self, caster: UnitState, effect, battlefield: BattlefieldState) -> Optional[Dict]:
        ...
        # target_identifier 解析: 优先使用 trigger context 中的目标
        # - "primary_target": 使用 _primary_target（AS 主目标，如 230169 バフシャット「自身が攻撃した対象」）
        # - "trigger_attacker": 使用 _trigger_attacker（反击型 PS）
        # - "triggered_by": 使用 _primary_target（PS 触发上下文目标，如 230170 デバフシャット「スキル対象の味方単体」）
        _bbt_tid = getattr(effect, 'target_identifier', None)
        targets = []
        if _bbt_tid == "primary_target":
            pt = getattr(self, '_primary_target', None)
            if pt and pt.is_alive:
                targets = [pt]
                _log.info("[BLOCK_BUFF_BY_TYPE] %s: primary_target -> %s", caster.name, pt.name)
            else:
                _log.info("[BLOCK_BUFF_BY_TYPE] %s: primary_target unavailable, no targets", caster.name)
                return None
        elif _bbt_tid == "trigger_attacker":
            ta = getattr(self, '_trigger_attacker', None)
            if ta and ta.is_alive:
                targets = [ta]
                _log.info("[BLOCK_BUFF_BY_TYPE] %s: trigger_attacker -> %s", caster.name, ta.name)
            else:
                _log.info("[BLOCK_BUFF_BY_TYPE] %s: trigger_attacker unavailable, no targets", caster.name)
                return None
        elif _bbt_tid == "triggered_by":
            pt = getattr(self, '_primary_target', None)
            if pt and pt.is_alive:
                targets = [pt]
                _log.info("[BLOCK_BUFF_BY_TYPE] %s: triggered_by -> %s", caster.name, pt.name)
            else:
                _log.info("[BLOCK_BUFF_BY_TYPE] %s: triggered_by target not available", caster.name)
                return None
        else:
            targets = self.target_service.select_targets(target_skill_obj, caster, battlefield)
        ...
        # block_debuffs flag: True = 阻止全debuff新付与（如141301 風紀委員会の管轄だよ～ L11+ デバフ無効）
        block_debuffs_flag = bool(bbf_flags.get('block_debuffs', False))
        # block_buffs flag: True = 阻止全buff新付与（如230169 バフシャット「対象に向けられるバフを無効にする」）
        block_buffs_flag = bool(bbf_flags.get('block_buffs', False))
        ...
        # action -> DURABLE_TARGET_MANEUVER_END (目标行动结束时-1)
        # attacker_action -> DURABLE_WHEN_USED (攻击者行动结束时由_cleanup_guard_buffs清除)
        if duration_type == 'source_action':
            timing = AuraUpdateTiming.DURABLE_SOURCE_MANEUVER_END.value
        elif duration_type == 'action':
            timing = AuraUpdateTiming.DURABLE_TARGET_MANEUVER_END.value
        elif duration_type == 'attacker_action':
            timing = AuraUpdateTiming.DURABLE_WHEN_USED.value
        else:
            timing = AuraUpdateTiming.DURABLE_SOURCE_MANEUVER_END.value

        # attacker_action时，记录触发攻击者unit_id用于cleanup
        trigger_attacker_id = ""
        if duration_type == 'attacker_action':
            _ta = getattr(self, '_trigger_attacker', None)
            if _ta:
                trigger_attacker_id = _ta.unit_id

        detail_list = []
        aura_details = []
        if block_debuffs_flag:
            blocked_display = "debuff"
        elif block_buffs_flag:
            blocked_display = "buff"
        else:
        ...
        for target in targets:
            ...
            aura = BuffState(
                ...
                block_debuffs=block_debuffs_flag,
                block_buffs=block_buffs_flag,
                triggered_by_attacker=trigger_attacker_id,
            ...
            _log.info("[BLOCK_BUFF_BY_TYPE] %s: applied to %s blocked_types=%s (enum=%s) block_debuffs=%s block_buffs=%s dur=%s dur_type=%s",
                      caster.name, target.name, blocked_types, sorted(blocked_enum_set),
                      block_debuffs_flag, block_buffs_flag, duration, duration_type)
    ...
    def _apply_reset_cooldown(self, caster: UnitState, effect) -> Optional[Dict]:
        ...
        if effect_condition and isinstance(effect_condition, dict):
            ...
            if cond_type == 'damage_target_has_status_ailment':
                # 检查本次技能的伤害目标是否有异常状态（炎上/毒/凍結/眩暈/黑暗/混乱/幻惑）
                # 異常状態≠debuff，異常状態只是debuff的子集
                STATUS_AILMENT_TYPES = {"knockout", "conflagration", "poison", "freeze",
                                        "darkness", "confusion", "genwaku"}
                bdt = getattr(self, '_block_damage_targets', None)
    ...
    def _apply_resource(self, caster: UnitState, effect, battlefield: BattlefieldState) -> Optional[Dict]:
        ...
        if etype == "add_ap":
            ...
            if ap_target_identifier == "trigger_attacker":
                ...
                elif ap_target_type and "lowest_atk" in ap_target_type and ap_targets:
                    ap_targets = [min(ap_targets, key=lambda u: self.damage_service._calculate_final_stat(u, "attack") if self.damage_service else u.attack)]
                    _log.info("[RESOURCE_EFFECT] %s: add_ap lowest_atk filter -> %s",
                              caster.name, ap_targets[0].name)
            if ap_targets:
                for t in ap_targets:
                    old_ap = t.current_ap
    ...
    def _apply_random_draw(self, caster: UnitState, effect, battlefield: BattlefieldState) -> Optional[Dict]:
        """随机抽取目标 effect (110072 ミッドサマー・ラブ)

        从候选池 {ally_single_exclude_self, enemy_single_nearest} 中随机抽取1个目标,
        重复 N 次 (N = effect.hit_count)。每次抽取独立判定，可重复抽到同一目标。
        抽取结果通过 self._current_drawn_targets 传递给后续 block (block 2/3 的 condition 检查)
        """
        import random
        if not self.target_service:
            return None
        flags = getattr(effect, 'flags', {}) or {}
        candidate_types = flags.get('candidate_types', [])
        draw_count = getattr(effect, 'hit_count', 1) or 1

        # 收集候选目标池
        candidates = []  # list of (unit, type_str)
        for ct in candidate_types:
            if ct == 'ally_single_exclude_self':
                ...
                # 选取距离 caster 最近的单个敌方
                alive_enemies = [u for u in opposing if u.is_alive]
                if alive_enemies:
                    nearest = self.target_service.get_nearest_enemy(caster, alive_enemies)
                    if nearest:
                        candidates.append((nearest, 'enemy'))

        if not candidates:
            _log.info("[RANDOM_DRAW] %s: no candidates, skip", caster.name)
            return None
        ...
        return {
            ...
            "caster_id": caster.unit_id,
            "draws": [{"target": u.name, "target_id": u.unit_id, "type": t} for u, t in drawn_list]
        }

    def _apply_dmg_taken_down_threshold(self, caster: UnitState, effect, battlefield: BattlefieldState) -> Optional[Dict]:
        """HP阈值减伤 buff (130160 サマータイム・ロマンス)

        仅超过 threshold (current_hp × threshold_pct%) 的伤害部分按 value% 减免。
        公式: actual = min(dmg, threshold) + max(0, dmg - threshold) × (1 - value/100)
        threshold = current_hp × threshold_pct / 100

        通过 buff 系统: effect_type='dmg_taken_down_threshold',
        value=减伤百分比, threshold_pct=阈值百分比, threshold_base=current_hp,
        hit_count = 减伤可生效的 hit 次数 (1/2/3)
        """
        # 通过 _apply_aura 走 buff 流程，但 effect_type 设为 'dmg_taken_down_threshold'
        # 需在 _get_buff_types() 中添加 'dmg_taken_down_threshold'
        return self._apply_aura(caster, effect, battlefield, is_debuff=False)
    ...
    def _check_target_condition(self, target: UnitState, effect_condition: dict) -> bool:
        ...
        if cond_type == 'target_has_status_ailment':
            # 異常状態≠debuff，只检查异常状态（炎上/毒/凍結/眩暈/黑暗/混乱/幻惑）
            STATUS_AILMENT_TYPES = {"knockout", "conflagration", "poison", "freeze",
                                    "darkness", "confusion", "genwaku"}
        ...
        if cond_type == 'target_ep_above_or_equal':
            # 120160 サマー・スプラッシュ: 检查目标EP是否>=value
            _ep_thresh = effect_condition.get('value', 0)
            _target_ep = getattr(target, 'current_ep', 0)
            _result = _target_ep >= _ep_thresh
            _log.info("[CHECK_CONDITION] %s: target_ep_above_or_equal ep=%.1f >= %s => %s",
                      target.name, _target_ep, _ep_thresh, _result)
            return _result
        return True
    ...
    def _resolve_target_range(self, effect_target_type: str) -> int:
        ...
        if t in ("enemy_column", "ally_column",
                 ...
                 "enemy_column_highest_atk",
                 "enemy_column_lowest_mark_count"): return DisplayTargetRange.COLUMN.value
    ...
    def _postfilter_damage_targets(self, target_type: str, targets: list,
        ...
        if target_type and "highest_atk" in target_type and "column_highest_atk" not in target_type:
            ...
            dmg_targets = [best] if best else []
        elif target_type and "lowest_atk" in target_type:
            best = self.target_service.select_min_with_stealth(
                dmg_targets,
                key_func=lambda u: self.damage_service._calculate_final_stat(u, "attack") if self.damage_service else u.attack,
                consume=consume_stealth
            ...
            dmg_targets = [best] if best else []
        elif target_type == "enemy_single_lowest_max_hp":
            best = self.target_service.select_min_with_stealth(
                dmg_targets,
                key_func=lambda u: (u.max_hp, self.target_service._get_sort_key(caster, u)),
                consume=consume_stealth
            ...
            if anchor is None:
                ...
                anchor_col = self.target_service._get_column_index(anchor)
                dmg_targets = [u for u in dmg_targets if self.target_service._get_column_index(u) == anchor_col]
        elif target_type == "enemy_column_lowest_mark_count":
            # 优先选择指定mark数量最少的敌方所在列（前后列/纵列）
            # 用于AS2 120122 容疑者を捕捉！「風紀」の数が最も少ない敵を優先
            mark_name = effect_flags.get('mark_priority', '風紀')
            def _mark_count_aura(u):
                return sum(1 for b in ((u.buffs or []) + (u.debuffs or []))
                          if getattr(b, 'name', '') == mark_name
                          and getattr(b, 'effect_type', '').lower() == 'mark')
            anchor = self.target_service.select_min_with_stealth(
                dmg_targets,
                key_func=lambda u: (_mark_count_aura(u), self._get_distance_key(caster, u)),
                consume=consume_stealth
            )
            if anchor is None:
                dmg_targets = []
    ...
    def _check_execute_condition(self, caster: UnitState, condition: Dict, battlefield: BattlefieldState) -> bool:
        """检查技能执行前置条件（如AS1 120121 HP>=60%才能发动）。
        ...
        if cond_type == 'self_hp_above':
            result = hp_pct >= cond_val
            _log.info("[EXEC_COND] %s: self_hp_above %.1f%% >= %s%% => %s",
                      caster.name, hp_pct, cond_val, result)
            return result
        elif cond_type == 'self_hp_below':
            result = hp_pct < cond_val
            _log.info("[EXEC_COND] %s: self_hp_below %.1f%% < %s%% => %s",
                      caster.name, hp_pct, cond_val, result)
            return result
        _log.warning("[EXEC_COND] unknown condition type: %s", cond_type)
        return True
    ...
    def _apply_consume_hp(self, caster: UnitState, effect, battlefield: BattlefieldState) -> Optional[Dict]:
        ...
        # value_tag解析：支持基于技能等级的动态百分比（如120067 自傷 Lv1-10=50%, Lv15=25%）
        if not pct and getattr(effect, 'value_tag', None):
            resolved = self._resolve_tag_value_for_caster(caster, effect, effect.value_tag)
            if resolved is not None:
                pct = resolved
                _log.info("[CONSUME_HP] %s: resolved value_tag '%s' -> %.1f (Lv%d)",
                          caster.name, effect.value_tag, pct,
                          caster.skill_levels.get(self._current_skill_id, 1))
    ...
    def _apply_heal_link(self, caster: UnitState, effect, battlefield: BattlefieldState) -> Optional[Dict]:
        """回復リンク: 对目标施加回復転送状态，対象受到回復时按転送率転送给施法者"""
        if not self.target_service:
            return None

        flags = getattr(effect, 'flags', {}) or {}
        transfer_pct = effect.value if effect.value is not None else 100.0
        duration = getattr(effect, 'duration', -1)
        duration_type = getattr(effect, 'duration_type', None) or "attacker_action"

        # 特殊索敌类型需用ALL_PAWNS获取全部候选再后过滤
        _HEAL_LINK_SPECIAL_TYPES = {
            "enemy_single_lowest_hp_ratio",
            "enemy_single_highest_hp_ratio",
            "enemy_single_highest_atk",
            "enemy_single_highest_spd",
        }
        _resolved_range = self._resolve_target_range(effect.target_type)
        if effect.target_type in _HEAL_LINK_SPECIAL_TYPES:
            from ...entities_v2.enums import DisplayTargetRange
            _resolved_range = DisplayTargetRange.ALL_PAWNS.value
        ...
        cached_targets = getattr(self, '_block_damage_targets', None)
        if cached_targets and effect.target_type in cached_targets:
            targets = [t for t in cached_targets[effect.target_type] if t.is_alive]
        else:
            targets = self.target_service.select_targets(target_skill_obj, caster, battlefield)
        ...
        if effect.target_type == "enemy_single_lowest_hp_ratio" and targets:
            ...
            targets = [best] if best else []
            _log.info("[HEAL_LINK] %s: lowest_hp_ratio filter -> %s",
                      caster.name, best.name if best else "none")

        if not targets:
            _log.info("[HEAL_LINK] %s: no targets for target_type=%s", caster.name, effect.target_type)
            return None

        from ...entities_v2.enums import AuraUpdateTiming
        timing = AuraUpdateTiming.DURABLE_SOURCE_MANEUVER_END.value
        if duration_type == "attacker_action":
            timing = AuraUpdateTiming.DURABLE_SOURCE_MANEUVER_END.value
        ...
        for target in targets:
            if not target.is_alive:
                continue
            aura = BuffState(
                buff_id=f"{caster.unit_id}_heal_link_{target.unit_id}",
                name="回復リンク",
                effect_type=SkillEffectType.HEAL_LINK.value,
                value=transfer_pct,
                duration=duration,
                timing_type=timing,
                source_unit_id=caster.unit_id,
    ...
    def _apply_hp_ratio_damage(self, caster: UnitState, effect, battlefield: BattlefieldState) -> Optional[Dict]:
        ...
        # damage_cap_atk_pct: 伤害上限为ATK的指定百分比（固定值）
        # damage_cap_atk_pct_tag: 伤害上限为ATK的指定百分比（按等级tag解析）
        cap_atk_pct = flags.get('damage_cap_atk_pct', 0)
        cap_atk_pct_tag = flags.get('damage_cap_atk_pct_tag', None)
        if cap_atk_pct_tag:
            resolved_cap = self._resolve_tag_value_for_caster(caster, effect, cap_atk_pct_tag)
            if resolved_cap is not None:
                cap_atk_pct = resolved_cap
                _log.info("[HP_RATIO_DMG] %s: damage_cap_atk_pct_tag='%s' resolved=%.1f",
                          caster.name, cap_atk_pct_tag, cap_atk_pct)
        ...
        _sub_unit_buffs, _add_damage_buffs, _enchant_buffs, _carried_debuff_buffs = \
            self._prepare_enchant_buffs_for_damage_effect(caster)
        # 捕获原始主目标（hp_ratio_damage 也需设置，供后续 block 的 damage_link self_and_primary_target 使用，如 110071）
        if getattr(self, '_original_primary_target', None) is None and targets:
            self._original_primary_target = targets[0]
            _log.info("[ORIGINAL_PRIMARY] %s: captured original primary target = %s (hp_ratio_damage)",
                      caster.name, targets[0].name)
        for target in targets:
            if not target.is_alive:
                continue
            ...
            # 护盾吸收（与普通damage一致：en_shield/physical_shield → shield → sub_unit_hp）
            # hp_ratio_damage默认可被盾吸收，ignore_shield控制穿盾比例
            shield_absorbed = 0
            shield_penetration = 0
            if getattr(effect, 'ignore_shield', 0):
                shield_penetration = min(effect.ignore_shield / 100.0, 1.0)
            ...
            hp_damage = shield_portion + direct_damage
            # 非闪避命中最低1点伤害，可作用于护盾或HP
            if hp_damage <= 0 and actual_damage > 0:
                min_absorbed = False
                if shield_penetration < 1.0:
                    caster_char_type = getattr(caster, 'character_type', 1)
                    is_en_damage = (caster_char_type == 2)
            ...
            caster.damage_dealt_total += hp_damage
            target.damage_taken_total += hp_damage

            # 计分追踪：记录HP比例伤害（hp_damage含溢出，全部计入得分）
            _scoring_tracker = getattr(battlefield, 'scoring_tracker', None)
            if _scoring_tracker is not None:
                _caster_side = "ally" if caster.side.value == "ally" else "enemy"
                _target_side = "ally" if target.side.value == "ally" else "enemy"
                _overflow = max(0, hp_damage - hp_before)
                _scoring_tracker.record_damage(
                    source_id=caster.unit_id, source_name=caster.name, source_side=_caster_side,
                    target_id=target.unit_id, target_name=target.name, target_side=_target_side,
                    actual_damage=hp_damage, shield_absorbed=shield_absorbed,
                    overflow=_overflow if _target_side == "enemy" else 0,
                )

            # RDPS 归因：HP比例伤害
            rdps_tracker = getattr(battlefield, 'rdps_tracker', None)
            if rdps_tracker is not None and hp_damage > 0:
                rdps_tracker.record_damage_with_attribution(
                    caster=caster, target=target, actual_damage=hp_damage,
            ...
            # per-target附魔处理（hp_ratio路径：crit固定False无hit_crits字段，force_crit将为False）
            self._process_enchant_damage_for_target(
                caster, targets_hit[-1], battlefield,
                _sub_unit_buffs, _add_damage_buffs,
                _enchant_buffs, _carried_debuff_buffs)
            ...
            shield_info = f" [shield={shield_absorbed}]" if shield_absorbed > 0 else ""
            dead_mark = " 💀DEAD" if target.current_hp <= 0 else ""
            _log.info("[HP_RATIO_DMG] %s -> %s: hp %d→%d (-%d)%s%s",
                      caster.name, target.name, hp_before, target.current_hp, hp_damage,
                      shield_info, dead_mark)
    ...
    def _apply_damage_special(self, caster: UnitState, effect, battlefield: BattlefieldState) -> Optional[Dict]:
        ...
        elif value_source == "consume_hp":
            # 基于本技能consume_hp消费的HP量（如AS1 120121 風紀を守る一撃）
            consumed = getattr(self, '_hp_consumed', 0)
            raw_damage = int(float(consumed) * dmg_pct / 100.0)
            effective_value = consumed
        else:
            effective_atk = self.damage_service._calculate_final_stat(caster, "attack")
        ...
        for target in targets:
            ...
            if can_evade:
                ...
                if evade_buffs and not getattr(target, 'is_charging', False):
                    ...
                    if sure_hit_buffs:
                        ...
                        evaded = True
                        # 技能级别闪避：同一技能内后续所有debuff效果也全部跳过
                        if not hasattr(self, '_skill_evaded_targets'):
                            self._skill_evaded_targets = set()
                        self._skill_evaded_targets.add(target.unit_id)
            ...
            if evaded:
                ...
                # per-target附魔处理（evaded路径：fully_evaded=True时跳过sub_unit/add_damage/enchant；
                # carried_debuff使用自身is_alive检查，目标仍存活故会执行）
                self._process_enchant_damage_for_target(
                    caster, targets_hit[-1], battlefield,
                    _sub_unit_buffs, _add_damage_buffs,
                    _enchant_buffs, _carried_debuff_buffs)
            ...
            # 计分追踪：记录特殊伤害（actual_dmg含溢出，全部计入得分）
            _scoring_tracker = getattr(battlefield, 'scoring_tracker', None)
            if _scoring_tracker is not None:
                _caster_side = "ally" if caster.side.value == "ally" else "enemy"
                _target_side = "ally" if target.side.value == "ally" else "enemy"
                _scoring_tracker.record_damage(
                    source_id=caster.unit_id, source_name=caster.name, source_side=_caster_side,
                    target_id=target.unit_id, target_name=target.name, target_side=_target_side,
                    actual_damage=actual_dmg, shield_absorbed=shield_absorbed,
                    overflow=overflow if _target_side == "enemy" else 0,
                )
            ...
            # per-target附魔处理（正常路径：crit固定False无hit_crits字段，force_crit将为False）
            self._process_enchant_damage_for_target(
                caster, targets_hit[-1], battlefield,
                _sub_unit_buffs, _add_damage_buffs,
                _enchant_buffs, _carried_debuff_buffs)
    ...
    def _check_force_crit_if_mark(self, caster: UnitState, effect_flags: dict) -> bool:
        """检查force_crit_if_mark条件：如果caster持有指定mark_name的mark buff则强制暴击

        用于技能120078 リミットブラスト: 持有「パワフル」mark时强制暴击
        """
        mark_name = effect_flags.get('force_crit_if_mark')
        if not mark_name:
            return False
        # 检查caster是否持有指定name的mark buff（is_buff_mark使mark存为buff）
        for buff in caster.buffs:
            if buff.name == mark_name and buff.effect_type == SkillEffectType.MARK.value:
                return True
        return False

    def _apply_reflect_damage(self, caster: UnitState, effect, battlefield: BattlefieldState) -> Optional[Dict]:
        """反射伤害效果：设置caster.reflect_rate，cover期间反射受到HP伤害的reflect_rate%给攻击者

        用于技能130088 ネガティブヘイト: cover期间反射受到伤害的50%/75%给攻击者
        反射伤害在_apply_damage中实际结算（与damage_link类似但独立实现）
        """
        # 从value_tag解析reflect值
        reflect_value = 0.0
        value_tag = getattr(effect, 'value_tag', None)
        if value_tag == "reflect":
            skill_id = self._current_skill_id
            meta = self.data_loader.get_skill_by_id(skill_id)
            if meta:
                skill_level = caster.skill_levels.get(skill_id, 1)
                tag_values = self._resolver._resolve_template_tags(meta, skill_level)
                reflect_value = tag_values.get('reflect', 0.0)

        if reflect_value <= 0:
            _log.info("[REFLECT_DAMAGE] %s: reflect value is %f, skip", caster.name, reflect_value)
            return None

        caster.reflect_rate = reflect_value
        _log.info("[REFLECT_DAMAGE] %s: reflect_rate=%.1f%% activated (cover period)",
                  caster.name, reflect_value)
        return {
            "effect_type": "reflect_damage",
            "reflect_rate": reflect_value,
            "caster_id": caster.unit_id,
        }
    ...
    def _apply_lifesteal(self, caster: UnitState, effect, battlefield: BattlefieldState) -> Optional[Dict]:
        ...
        # target_type扩展：当配置了target_type (如ally_single) + lowest_hp_priority时
        # 吸血治疗目标改为HP比例最低的友方（不含自身），而非施法者自身
        # 注意：按HP百分比（current_hp/max_hp）而非HP绝对值选择
        ls_flags = getattr(effect, 'flags', {}) or {}
        ls_target_type = getattr(effect, 'target_type', None)
        target = caster  # 默认目标为施法者自身
        if ls_target_type in ("ally_single", "ally_all", "friend", "friends") and ls_flags.get('lowest_hp_priority'):
            from src.entities_v2.enums import Side as _SideLS
            ally_team = battlefield.friend_team if caster.side == _SideLS.ALLY else battlefield.enemy_team
            others = [u for u in ally_team if u.is_alive and u.unit_id != caster.unit_id]
            if others:
                # 按HP百分比升序，tiebreaker为距离施法者最近
                others.sort(key=lambda u: (u.current_hp / max(u.max_hp, 1),
                                           self.target_service._get_sort_key(caster, u)))
                target = others[0]
                _log.info("[LIFESTEAL] %s: lowest_hp_priority -> %s (hp_pct=%.1f%%)",
                          caster.name, target.name,
                          target.current_hp / max(target.max_hp, 1) * 100)
            elif caster.is_alive:
                target = caster
            else:
                _log.info("[LIFESTEAL] %s: no valid ally target, skip", caster.name)
                return None
        ...
        if heal_received_mult != 1.0:
            ...
            _log.info("[LIFESTEAL_EFFICACY] %s: heal_efficacy_mult=%.4f heal_amount=%d",
                      target.name, heal_received_mult, heal_amount)

        hp_before = target.current_hp
        effective_max_hp = self.damage_service._calculate_final_stat(target, "max_hp")
        target.current_hp = min(effective_max_hp, target.current_hp + heal_amount)
        actual_heal = target.current_hp - hp_before

        _log.info("[LIFESTEAL] %s -> %s: healed %d (%.0f%% of %d dmg, efficacy=%.4f), hp %d→%d",
                  caster.name, target.name, actual_heal, cure_pct, recent_dmg, heal_received_mult,
                  hp_before, target.current_hp)
